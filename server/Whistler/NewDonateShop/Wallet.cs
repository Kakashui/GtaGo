...
namespace Whistler.NewDonateShop
{
    public abstract class Wallet
    {
        ...
        public void OrderCoinKit(PlayerGo player, int id)
        {
            try
            {
                ...
                player.TriggerEvent("dshop:wallet:redirect", link);
            }
            catch (Exception ex)
            {
        ...
        public void OrderCoins(PlayerGo player, int count)
        {
            try
            {
                if (count < 1) return;
        ...
        public void ExchangeCoinsToMoney(PlayerGo player, int amount)
        {
            if (amount < 1) return;
            ...
            if (player.Account.GoCoins < price)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "dshop:item:buy:wrong:coins".Translate(), 3000);
                player.UpdateCoins();
                return;
            }
            player.SubGoCoins(price);
            MoneySystem.Wallet.MoneyAdd(player.GetCharacter(), amount, "Money_DonateConvert");
