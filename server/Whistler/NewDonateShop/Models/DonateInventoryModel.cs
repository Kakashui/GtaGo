...
namespace Whistler.NewDonateShop.Models
{
    public class DonateInventoryModel
    {
        ...
        public void UseItem(PlayerGo player, int id, bool sell, int count)
        {
        ...
        public void SellItem(PlayerGo player, int id, bool sell)
        {
            var item = DonateService.Items[id];
        ...
        public void AddItem(int id, bool sell, bool update = true)
        {
            ...
            if (update)
            {
                ...
                {
                    player.TriggerEvent("dshop:inventory:update", i.Id, i.Count, i.Sell);
                }
            }
        }
        ...
        public bool RemoveItem(int id, bool sell, int count = 1)
        {
            ...
            if (i.Count < 1)
            {
                ...
                foreach (var player in _subscribes)
                {
                    player.TriggerEvent("dshop:inventory:update", i.Id, -1, i.Sell);
                }
            }
            else
            {
        ...
        public void UpdateItemDataForPlayer(Player player)
        {
            player.TriggerEvent("dshop:inventory:set", GetItemData());
        }
