...
namespace Whistler.Houses.Models
{
    class Garage
    {
        ...
        public bool GetIsPlayerCanEnterGarage(PlayerGo player, bool fromStreet)
        {
            if (!player.IsLogged())
                return false;
        ...
        public void SpawnCarAtPosition(int idkey, Vector3 position, Vector3 rotation, uint dimension = 0, int place = -1)
        {
            try
            {
                if (!spawnedVehicles.ContainsKey(idkey))
                {
                    Vehicle veh = VehicleManager.Vehicles[idkey].Spawn(position, rotation, dimension);
                    if (veh != null)
        ...
        public void ExitGarageIntoVehicle(PlayerGo player)
        {
            ...
            if (!spawnedVehicles.ContainsKey(vehGo.Data.ID))
                return;
            spawnedVehicles[vehGo.Data.ID].Place = -1;

            player.GetCharacter().InsideGarageID = -1;
        ...
        public void SendPlayer(Player player)
        {
            ...
            player.GetCharacter().InsideGarageID = ID;
            if (GarageConfig.IPLType > GarageIPLType.NoIPL)
                player.TriggerEvent("garage:loadInteriors", GarageConfig.CoordsModel.Position, (int)GarageConfig.IPLType);
            RespawnCars();
        }
        ...
        public bool EnterGarage(PlayerGo player, bool fromStreet)
        {
            ...
            {
                var vehicles = VehicleManager.getAllHolderVehicles(house.OwnerID, house.OwnerType);
                if (!vehicles.Contains(player.Vehicle.GetVehicleGo().Data.ID))
                {
                ...
                {
                    GarageManager.SendVehicleIntoGarage(player.Vehicle.GetVehicleGo().Data);
                }
            }
            SendPlayer(player);
            return true;
