...
namespace Whistler.Houses
{
    class HouseManager : Script
    {
        ...
        public static Action<Player, House> PlayerEntered;
        public static Action<Player> PlayerLeaved;
        ...
        public static void Event_OnPlayerDeath(Player player, Player entityKiller, uint weapon)
        {
            try
            {
                GetHouseById(player.GetCharacter().InsideHouseID)?.RemovePlayer(player);
            }
        ...
        public static void Event_OnPlayerDisconnected(Player player, DisconnectionType type, string reason)
        {
            try
            {
                GetHouseById(player.GetCharacter().InsideHouseID)?.RemoveFromList(player);
            }
        ...
        {
            House house = Houses.FirstOrDefault(h => h.OwnerType == OwnerType.Personal && h.OwnerID == player.GetCharacter().UUID);
            if (house != null)
                return house;
            else if (!checkOwner)
                return Houses.FirstOrDefault(h => h.GetRoommate(player.GetCharacter().UUID) != null);
            else
                return null;
        }
        public static House GetHouseFamily(Player player)
        {
            if (player.GetCharacter().FamilyID == 0)
                return null;
            return Houses.FirstOrDefault(h => h.OwnerID == player.GetCharacter().FamilyID && h.OwnerType == OwnerType.Family);
        }
        ...
        public static void CheckAndKick(Player player)
        {
            ...
            GetHouse(player)?.RemoveRoommate(player.GetCharacter().UUID);
        }
        #endregion

        [RemoteEvent("houses:buy")]
        public static void Houses_BuyEvent(Player player)
        {
            try
            {
                ...
                DialogUI.Open(player, "houses_3", new List<DialogUI.ButtonSetting>
                {
                    new DialogUI.ButtonSetting
                    {
                        Name = "houses_4",
                        Icon = null,
                        Action = (p) => BuyHouse(p, house, OwnerType.Personal),
                    },
                    ...
                    new DialogUI.ButtonSetting
                    {
                        Name = "houses_5",
                        Icon = null,
                        Action = (p) => BuyHouse(p, house, OwnerType.Family),
                    },
        ...
        public static void BuyHouse(Player player, House house, OwnerType typeOwner)
        {
            if (house.OwnerID != -1)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "House_28", 3000);
                return;
            }
            var character = player.GetCharacter();
            if (!MoneySystem.Wallet.TryChange(character, -house.Price))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "House_29", 3000);
                return;
            }
            switch (typeOwner)
            {
                case OwnerType.Personal:
                    if (Houses.Count(h => h.OwnerID == character.UUID) >= 1)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "House_30", 3000);
                        return;
                    }
                    Notify.Alert(player, "House_32");
                    CheckAndKick(player);
                ...
                case OwnerType.Family:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "House_130", 3000);
                        return;
                    }
                    if (Houses.Count(h => h.OwnerID == character.FamilyID) >= 1)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "House_131", 3000);
                        return;
                    }
                    Notify.Alert(player, "House_132");
            ...
            MoneySystem.Wallet.MoneySub(character, house.Price, "Money_HouseBuy".Translate((int)typeOwner, house.ID));
        }

        [RemoteEvent("houses:enter")]
        public static void Houses_EnterEvent(Player player)
        {
            try
            {
                ...
                else
                {
                    if (house.Locked)
                    {
                        ...
                        var familyHouse = GetHouseFamily(player);
                        if (playerHouse != null && playerHouse.ID == house.ID)
                            house.SendPlayer(player);
                        else if (familyHouse != null && familyHouse.ID == house.ID)
                            house.SendPlayer(player);
                        else
        ...
        public static void Houses_BreakTheDoor(PlayerGo player)
        {
            if (player.IsInVehicle) return;
            if (!player.HasData("HOUSEID")) return;
        ...
        public static void Houses_TheDoorIsBroken(PlayerGo player)
        {
        ...
        public static void OnOwnerMenuOpenedFromMMenu(Player player)
        {
            try
            {
                GetHouse(player, true)?.OnOwnerInteracted(player);
            }
        ...
        public static void OpenFamilyHouseMenu(Player player)
        {
            try
            {
                player.GetFamily()?.GetHouse()?.OnOwnerInteracted(player);              
            }
            catch (Exception e) { _logger.WriteError($"Exception catched on 'houses:changeAccess' event: {e.ToString()}"); }
        }
        ...
        {
            if (player.GetCharacter().InsideHouseID == -1) return;
        ...
        public static void ExitToGarage(PlayerGo player)
        {
            if (player.GetCharacter().InsideHouseID == -1) return;
        ...
        public static void ExitToStreet(Player player)
        {
            GetHouseById(player.GetCharacter().InsideHouseID)?.RemovePlayer(player);
        }
        ...
        public static void OfferHouseSell(Player player, Player target, int price)
        {
            ...
            DialogUI.Open(target, "House_123".Translate(player.GetCharacter().UUID, price), new List<DialogUI.ButtonSetting>
            {
                new DialogUI.ButtonSetting
                {
                    Name = "gui_727",
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "House_124".Translate( target.GetCharacter().UUID, price), 3000);
        }

        public static void acceptHouseSell(Player player, Player seller, int price)
        {
            ...
            if (house == null) return;
            if (!MoneySystem.Wallet.TransferMoney(player.GetCharacter(), seller.GetCharacter(), price, 0, "Money_HouseSell".Translate(house.ID)))
            {
            ...
            house.SetOwner(player.GetCharacter().UUID, 0);

            MainMenu.SendProperty(player);
            MainMenu.SendProperty(seller);

            Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter, "House_127".Translate(player.GetCharacter().UUID), 3000);
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "House_128".Translate( seller.GetCharacter().UUID), 3000);
        }
    }
}
