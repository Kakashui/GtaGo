...
namespace Whistler.Jobs.CarThief
{
    ...
    {
        public Thief(PlayerGo client) : base(client)
        {
        }
        ...
        public void StartWorkQuest()
        {
            try
            {
                ...
                Player.TriggerEvent("WORK::CARTHIEF::PED::CRETATE", Agent.Hash, Agent.Position.X, Agent.Position.Y, Agent.Position.Z, Agent.Rotation, 0);
        ...
        public void EndWorkQuest()
        {
            try
            {
                ...
                Player.TriggerEvent("WORK::CARTHIEF::CHANGE::STATE::SERVER", false);
                Player.TriggerEvent("WORK::CARTHIEF::CHANGE::LOCK::SERVER", false);
        ...
        public void SendSalaryToBank()
        {
            try
            {
                ...
                int salary = DonateService.UseJobCoef(Player, Convert.ToInt32(Work.PriceByDistance * resultDistance));
                EarnedMoney += salary;
                MoneySystem.Wallet.MoneyAdd(Player.GetBankAccount(), salary, "Money_CarThief");
                if ((Player.GetFraction()?.OrgActiveType ?? OrgActivityType.Invalid) == OrgActivityType.Crime)
                    MoneySystem.Wallet.MoneyAdd(Player.GetFraction(), salary, "Money_Work");
                else
                {
                    if ((Player.GetFamily()?.OrgActiveType ?? OrgActivityType.Unknown) == OrgActivityType.Crime)
                        MoneySystem.Wallet.MoneyAdd(Player.GetFamily(), salary, "Money_Work");
                }
        ...
        public void StartCarSearch()
        {
            try
            {
                ...
                this.Player.SetData("lastWorkAction", DateTime.Now);
                ...
                Player.TriggerEvent("WORK::CARTHIEF::CHANGE::STATE::SERVER", true); // Запускаем, чтобы срабатывала кнопка O
                Player.TriggerEvent("WORK::CARTHIEF::CHANGE::LOCK::SERVER", false); // Замок автомобиля закрыт, чтобы не могли завести
        ...
        public void EndCarSearch()
        {
            try
            {
                ...
                this.Player.TriggerEvent("WORK::CARTHIEF::CHANGE::LOCK::SERVER", true);
        ...
        public void StartNumberChange()
        {
            try
            {
                if (this.JobStage.Equals(QuestStage.CAR_NUMER_CHANGE))
                {
                    ...
                    this.Player.TriggerEvent("WORK::CARTHIEF::DIAGNOSTIC::SERVER");
                }
            }
            catch (Exception ex)
            {
        ...
        public void ChangeNameColor(bool isRed)
        {
            try
            {
                this.Player.SetSharedData("REDNAME", isRed);
            }
            catch (Exception ex)
            {
        ...
        public void CreateRandomMapZone(Vector3 location, int radius)
        {
            try
            {
                ...
                Player.TriggerEvent("WORK::CARTHIEF::CREATE::RADIUS", random, radius, Work.MapZoneColor);
        ...
        public void DeleteExistedMapZone()
        {
            try
            {
                Player.TriggerEvent("WORK::CARTHIEF::DELETE::RADIUS");
            }
            catch (Exception ex)
            {
