...
namespace Whistler.Jobs.CarThief
{
    ...
    {
        public Policeman(PlayerGo client) : base(client) { }
        ...
        public void DeliverCar()
        {
            try
            {
                ...
                if (!this.Player.IsInVehicle || !this.Car.Vehicle.Occupants.Contains(this.Player) || this.Car.Vehicle.Position.DistanceTo(DeliverPoint) > 3) return;
        ...
        public void SendSalaryToBank()
        {
            try
            {
                ...
                MoneySystem.Wallet.MoneyAdd(Player.GetBankAccount(), salary, "Money_CarThiefLSPD");
