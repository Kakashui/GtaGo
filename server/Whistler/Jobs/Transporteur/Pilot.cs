...
namespace Whistler.Jobs.Transporteur
{
    ...
    {
        public Pilot(PlayerGo client) : base(client)
        {
        ...
        public async void GiveCarAsync()
        {
            try
            {
                ...
                while (!resultOfCreateCar)
                {
                    ...
                    {
                        WhistlerTask.Run(()=> resultOfCreateCar = this.GiveCar());
                    }
        ...
        public bool GiveCar()
        {
            try
            {
                ...
                */ 
                ...
                this.Player.TriggerEvent("WORK::TRANSPORTEUR::CREATE::VEHICLE::SERVER", randomCarLocation.Position.X, randomCarLocation.Position.Y, randomCarLocation.Position.Z, 
                ...
                this.Player.TriggerEvent("WORK::TRANSPORTEUR::CHANGE::STATE::SERVER", "isFlyToCar", true);
                ...
                this.TimerID = Timers.StartOnce(minutes * 60 * 1000, () => WhistlerTask.Run(() => this.CheckTimer(QuestStage.FLY_TO_CAR)));
        ...
        public async void EndWorkQuest()
        {
            try
            {
                ...
                WhistlerTask.Run(() => {
                    Player.TriggerEvent("WORK::TRANSPORTEUR::DELETE::VEHICLE::SERVER");
                    // Платим зарплату
                    SendSalaryToBank();
                });
        ...
        public void GiveDeliverPoint()
        {
            try
            {
                ...
                Player.TriggerEvent("WORK::TRANSPORTEUR::CHANGE::STATE::SERVER", "isFlyToCar", false);
                ...
                this.StartClientTimer(minutes);
                // Запускаем таймер на сервере
                this.TimerID = Timers.StartOnce(minutes * 60 * 1000, () => WhistlerTask.Run(() => this.CheckTimer(QuestStage.DELIVER)));
        ...
        public void SendSalaryToBank()
        {
            try
            {
                int salary = DonateService.UseJobCoef(Player, Salary);
                MoneySystem.Wallet.MoneyAdd(Player.GetBankAccount(), salary, "Money_Transporteur");
        ...
        public void CreateCheckpoint(Vector3 point, int scale = 1)
        {
            try
            {
                Trigger.ClientEvent(Player, "createCheckpoint", 15, 1, point, scale, 0, 255, 0, 0);
                Trigger.ClientEvent(Player, "createWorkBlip", point);
            }
            catch (Exception ex)
            {
        ...
        public void CheckTimer(QuestStage questStageCheck)
        {
            try
            {
                ...
                // Запускаем таймер
                //this.Player.TriggerEvent("WORK::TRANSPORTEUR::TIMER::SET::SERVER", timerSeconds);
                ...
                {
                    this.Player.GetCharacter().WorkID = 0;
                    this.Dispose();
        ...
        public void StopTimer()
        {
            try
            {
                ...
                this.Player.TriggerEvent("WORK::TRANSPORTEUR::TIMER::STOP::SERVER");
                if (this.TimerID.Equals("")) return;
                // Отключаем таймер, чтобы он не сработал
                Timers.Stop(this.TimerID);
            }
            catch (Exception ex)
            {
        ...
        public void Dispose()
        {
            try
            {
                ...
                Player.TriggerEvent("WORK::TRANSPORTEUR::CHANGE::STATE::SERVER", "isTransporteurWorker", false);
                Player.TriggerEvent("WORK::TRANSPORTEUR::CHANGE::STATE::SERVER", "isFlyToCar", false);
                ...
                Player.SetData($"{Work.JobName}::WORK::LEAVE::RECENTLY", true);
                // Даем возможность устроиться на работу через указанное в настройках время
                WhistlerTask.Run(() =>
                {
                    // Даем возможность устроиться
                    Player.SetData($"{Work.JobName}::WORK::LEAVE::RECENTLY", false);
