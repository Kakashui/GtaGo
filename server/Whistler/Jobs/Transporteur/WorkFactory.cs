...
namespace Whistler.Jobs.Transporteur
{
    ...
    {
        public WorkFactory(int minLVL, int workID, int workTimer, string jobName, Vector3 startEndWorkPoint, uint blipID, byte colorID, Action<PlayerGo> startWorkAction) 
            : base(minLVL, workID, workTimer, jobName, startEndWorkPoint, blipID, colorID, startWorkAction)
        {
        }

        public override AbstractWorker CreateWorker(PlayerGo client)
        {
            try
            {
