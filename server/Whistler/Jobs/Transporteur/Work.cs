...
namespace Whistler.Jobs.Transporteur
{
    class Work : Script
    {
        ...
        public static int MinLVL { get; set; } = 15;
        ...
        public void onResourceStartHandler()
        {
            try
            {
                ...
                foreach (Vector3 deliverPoint in DeliverPoints)
                {
                    ...
                    colsh.OnEntityEnterColShape += (colshape, player) =>
                    {
                        try
                        {
                            ...
                            player.TriggerEvent("WORK::TRANSPORTEUR::DELIVER::VEHICLE::SERVER");
                        }
        ...
        public static void onPlayerDisconnected(Player player, DisconnectionType type, string reason)
        {
            try
            {
                ...
                if (worker != null)
                {
                    ...
                    //worker.Player.TriggerEvent("WORK::TRANSPORTEUR::DELETE::VEHICLE::SERVER");
        ...
        public static void PilotEndDeliver(Player player)
        {
            try
            {
        ...
        public static void PilotStartDeliver(Player player)
        {
            try
            {
                // Если такого игрока нет на сервере - прерываем
        ...
        public static void PilotStartEngine(Player player)
        {
            try
            {
                // Если такого игрока нет на сервере - прерываем
                ...
                pilot.Player.TriggerEvent("WORK::TRANSPORTEUR::MAGNET::VEHICLE::SERVER");
                ...
                //          if (pilot.Cargobob.Vehicle.Occupants.Contains(pilot.Player) && pilot.Player.Position.DistanceTo(pilot.Cargobob.CarLocation.Position) > 20)
        ...
        public static void InitWorkDay(PlayerGo client)
        {
            try
            {
                ...
                if (!Workers.Exists(e => e.Player == client))
                {
                    ...
                    client.GetCharacter().WorkID = Work.WorkID;
                    ...
                    worker.Player.TriggerEvent("WORK::TRANSPORTEUR::CHANGE::STATE::SERVER", "isTransporteurWorker", true);
                    ...
                    worker.TimerID = Timers.StartOnce(minutes * 60 * 1000, () => WhistlerTask.Run(() => worker.CheckTimer(QuestStage.GET_HELICOPTER)));
                ...
                else
                {
                    ...
                    worker.Player.GetCharacter().WorkID = 0;
                    worker.Dispose();
            ...
            {
                NAPI.Util.ConsoleOutput($"Work.InitWorkDay(Player pilot): {ex.ToString()}");
            }
        }
