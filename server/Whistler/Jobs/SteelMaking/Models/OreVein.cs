...
{    class OreVein
    {
        ...
        public OreVein(int point, Vector3 position)
        {
            ...
            Shape.OnEntityEnterColShape += (ColShape colShape, Player player) => EnterOreVeinColshapeHandler((PlayerGo)player);
            Shape.OnEntityExitColShape += (ColShape colShape, Player player) => ExitOreVeinColshapeHandler((PlayerGo)player);
        ...
        public void Destroy()
        {
            ...
            _marker?.Delete();
            _blip?.Delete();
            Trigger.ClientEventInRange(Position, OreMiningSettings.VeinRange + 4, "OreVein:ExitPoint", Point);
        }

        private void EnterOreVeinColshapeHandler(PlayerGo player)
        {
            try
            {
                player.OreVeinID = Point;
                Trigger.ClientEvent(player, "OreVein:EnterPoint", Point, Position);
            }
            catch (Exception e) { _logger.WriteError("Unhandled error on EnterInteractColshapeHandler: " + e.ToString()); }
        }

        private void ExitOreVeinColshapeHandler(PlayerGo player)
        {
            try
            {
                ...
                {
                    player.OreVeinID = -1;
                    Trigger.ClientEvent(player, "OreVein:ExitPoint", Point);
                }
            }
        ...
        public void ExplodeDynamite(Vector3 position)
        {
            ...
            WhistlerTask.Run(() =>
            {
                ...
                Trigger.ClientEventInRange(position, 200, "OreVein:explodeDynamit", position, Point, CountOresDrop);
                WhistlerTask.Run(() =>
                {
                    ...
                    {
                        Trigger.ClientEvent(client, "OreVein:GetDropPositions", Position, Point, CountOresDrop);
                    };
                }, 6000);
            }, 10000);
        }
