...
namespace Whistler.Jobs.SteelMaking
{
    class OreMining : Script
    {
        ...
        //[Command("giveother")]
        //public static void CMD_GiveDynamit(PlayerGo player, int id)
        //{
        ...
        public static bool OnUseDynamite(PlayerGo player, Other dynamite)
        {
            ...
            Trigger.ClientEventInRange(pos, 200, "OreVein:explosionDynamitTimer", player.Position, 10, player.OreVeinID);
            _oreVein[player.OreVeinID]?.ExplodeDynamite(pos);
            player.CreatePlayerAction(PersonalEvents.PlayerActions.ExplodeDynamite, 1);
            return true;
        }
        public static void OnUseDetector(PlayerGo player)
        {
            Trigger.ClientEvent(player, "OreVein:UseDetector", true);
        }
        public static void OnStopUseDetector(PlayerGo player)
        {
            Trigger.ClientEvent(player, "OreVein:UseDetector", false);
        }

        [RemoteEvent("OreVeid:CreateDrops")]
        public static void RemoteEvent_CreateDrops(Player player, int point, string dataPos)
        {
