...
namespace Whistler.Jobs.Technician
{
    class Work : Script
    {
        ...
        public static void InitWorkDay(PlayerGo client)
        {
            try
            {
                ...
                if (!Workers.Exists(e => e.Player == client))
                {
                    ...
                    if (client.GetCharacter().WorkID != 0 && client.GetCharacter().WorkID != Work.WorkID)
                    {
                ...
                else
                {
                    ...
                    client.SetData("TECHNICIAN::WORK::LEAVE::RECENTLY", true);
                    // Даем возможность устроиться на работу через указанное в настройках время
                    WhistlerTask.Run(() =>
                    {
                        // Даем возможность устроиться
                        client.SetData("TECHNICIAN::WORK::LEAVE::RECENTLY", false);
        ...
        [Command("techworkers")]
        public static void CMD_GetLobbiesActive(Player player)
        {
