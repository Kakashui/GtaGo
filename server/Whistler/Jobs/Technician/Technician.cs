...
namespace Whistler.Jobs.Technician
{
    ...
    {
        public Technician(PlayerGo client) : base(client) { }
        ...
        public void StartDiagnostic()
        {
            try
            {
                ...
                Player.TriggerEvent("WORK::TECHNICIAN::DIAGNOSTIC::SERVER");
            }
            catch (Exception ex)
            {
        ...
        public void EndDiagnostic(bool isSuccess)
        {
            try
            {
                ...
                if (!isSuccess) return;
                Player.SetData("lastWorkAction", DateTime.Now);
        ...
        public void SendSalaryToBank()
        {
            try
            {
                ...
                int salary = DonateService.UseJobCoef(Player, Convert.ToInt32(resultDistance * PriceByDistance));
                EarnedMoney += salary;
                MoneySystem.Wallet.MoneyAdd(Player.GetBankAccount(), salary, "Money_TechWork");
        ...
        public override void CreateWaypoint(Vector3 point)
        {
            try
            {
                ...
                Trigger.ClientEvent(Player, "createCheckpoint", 15, 1, point, 1, 0, 255, 0, 0);
