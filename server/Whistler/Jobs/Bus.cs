...
namespace Whistler.Jobs
{
    class Bus : Script
    {
        ...
        public void StartCreatingBusWay(Player player)
        {
            try
            {
                ...
                player.SetData("BUSWAY_CHECKPOINTS", checkpoints);
        ...
        [Command("addbuspoint")]
        public void AddNewBusWayPoint(Player player, bool isStop)
        {
            try
            {
                ...
                player.TriggerEvent("buswayscreator:pushMarker", position, isStop);
            }
            catch (Exception e) { _logger.WriteError("addbuspoint: " + e.ToString()); }
        }

        [Command("removebuspoint")]
        public void RemoveLastBusWayPoint(Player player)
        {
            try
            {
                ...
                player.TriggerEvent("buswayscreator:popMarker");
            }
            catch (Exception e) { _logger.WriteError("removebuspoint: " + e.ToString()); }
        }

        [Command("finishbusway")]
        public void FinishBusWay(Player player, string busWayName)
        {
            try
            {
                ...
                player.TriggerEvent("buswayscreator:clearMarkers");
            }
        ...
        private static void CreateBusQuestPed()
        {
            ...
            questPed.PlayerInteracted += (player, ped) =>
            {
                try
                {
                    ...
                    if (player.GetCharacter().WorkID == 4)
        ...
        public static uint BusHash = NAPI.Util.GetHashKey("tourbus");
        
        private static void StartWork(Player player)
        {
            ...
            player.GetCharacter().WorkID = 4;            
            var vehicle = VehicleManager.CreateTemporaryVehicle(BusHash, firstUnusedSpot.Position, new Vector3(0, 0, firstUnusedSpot.Heading), "BUS", VehicleAccess.WorkBus, player);
            player.TriggerEvent("bus:vehicleLoaded", JsonConvert.SerializeObject(vehicle.Position));
        ...
        private static void busCheckpointEnterWay(ColShape shape, Player player)
        {
            try
            {
                ...
                if (player.GetCharacter().WorkID != 4 || !player.GetData<bool>("ON_WORK") || player.GetData<int>("WORKWAY") != shape.GetData<int>("WORKWAY")) return;
                ...
                if (!BusWays[way][check].IsStop)
                {
                    ...
                    var payment = DonateService.UseJobKoef(player, BuswaysPayments[way]);
                    ...
                    Trigger.ClientEvent(player, "bus:createCheckpoint", 
                        JsonConvert.SerializeObject(BusWays[way][check].Pos - new Vector3(0, 0, 1.12)),
                        color.Red, color.Green, color.Blue);

                    player.SetData("WORKCHECK", check);
                    MoneySystem.Wallet.MoneyAdd(player.GetCharacter(), payment, "Money_BusPayment");
                }
                else
                {
                    if (player.GetData<int>("WORKCHECK") != check) return;
                    Trigger.ClientEvent(player, "bus:clear", 3, 0);

                    player.SetData("BUS_ONSTOP", true);
        ...
        private static void timer_busStop(Player player, int way, int check)
        {
            ...
            var payment = DonateService.UseJobCoef(player, BuswaysPayments[way]);
            ...
            NAPI.ClientEvent.TriggerClientEvent(player, "bus:createCheckpoint", 
                JsonConvert.SerializeObject(BusWays[way][check].Pos - new Vector3(0, 0, 1.12)),
                color.Red, color.Green, color.Blue);

            player.SetData("WORKCHECK", check);
            //Main.StopT(player.GetData<string>("BUS_TIMER"), "timer_23");
        ...
        public static void OnStartWork(Player player)
        {
            if (player.GetCharacter().Money >= BusRentCost)
            {
                DialogUI.Open(player, $"Jobs_167".Translate(BusRentCost), new List<DialogUI.ButtonSetting>
                {
                    ...
                    new DialogUI.ButtonSetting
                    {
                        Name = "House_59",// no
                        Icon = "cancel",
                        Action = VehicleManager.WarpPlayerOutOfVehicle
                    }
                });
            }
            else
        ...
        public static void acceptBusRent(Player player)
        {
            if (NAPI.Player.IsPlayerInAnyVehicle(player) && player.VehicleSeat == VehicleConstants.DriverSeat)
            {
                ...
                if (vehGo.Data.OwnerType != OwnerType.Temporary || (vehGo.Data as TemporaryVehicle).Access != VehicleAccess.WorkBus || (vehGo.Data as TemporaryVehicle).Driver != player) 
                {
                ...
                MoneySystem.Wallet.MoneySub(player.GetCharacter(), BusRentCost, "Money_BusRentCost");
                ...
                Trigger.ClientEvent(player, "bus:createCheckpoint", JsonConvert.SerializeObject(BusWays[way][0].Pos - new Vector3(0, 0, 1.12)), 0, 255, 0);
