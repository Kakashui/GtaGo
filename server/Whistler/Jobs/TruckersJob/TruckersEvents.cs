...
namespace Whistler.Jobs.TruckersJob
{
    internal class TruckersEvents : Script
    {
        ...
        public void OnPlayerEnteredCheckpoint(PlayerGo player)
        {
            try
            {
                var vehicle = player.Vehicle;
                if (vehicle == null || vehicle.GetVehicleGo().Data.OwnerType != OwnerType.Temporary || (vehicle.GetVehicleGo().Data as TemporaryVehicle).Access != VehicleAccess.WorkTruck) return;
        ...
        public void OnPlayerSettedTruck(PlayerGo player, int id)
        {
            try
            {
        ...
        [Command("trucklevel")]
        public void SetTruckLevelCommand(PlayerGo player, int id, int level)
        {
            try
            {
                if (!Group.CanUseAdminCommand(player, "trucklevel")) return;
                var target = Main.GetPlayerByID(id);
                if (!target.GetCharacter().ImprovableJobs.ContainsKey(ImpovableJobs.ImprovableJobType.ProductsLoader))
                {
                    target.GetCharacter().ImprovableJobStates.Add(new ImpovableJobs.ImprovableJobState(ImpovableJobs.ImprovableJobType.ProductsLoader));
                }
                else
                {
                    // Делаем дальнобойщиком для начал
                    target.GetCharacter().ImprovableJobs[ImpovableJobs.ImprovableJobType.ProductsLoader].CurrentLevel = 1;
                    var stage = TruckersJobSettings.Stages.FirstOrDefault(s => TruckersJobSettings.GetLevel(s.Value.RequiredTransportations) == level).Key - 1;
                    if (!TruckersJobSettings.Stages.ContainsKey(stage))
                    {
                        Notify.Send(player, NotifyType.Info, NotifyPosition.Bottom, "job:truck:1", 3000);
                        return;
                    }
                    target.GetCharacter().ImprovableJobs[ImpovableJobs.ImprovableJobType.ProductsLoader].StagesPassed = TruckersJobSettings.Stages[stage].RequiredTransportations + 1;
                    Notify.Alert(target, "Truckers_26");
                }
            }
