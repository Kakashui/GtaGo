...
namespace Whistler.Jobs.AbstractEntity
{
    ...
    {
        public AbstractFactory(int minLVL, int workID, int workTimer, string jobName, Vector3 startEndWorkPoint, uint blipID, byte colorID, Action<PlayerGo> startWorkAction)
        {
            this.MinLVL = minLVL;
            this.WorkID = workID;
        ...
        public abstract AbstractWorker CreateWorker(PlayerGo client);

        public abstract AbstractVehicle CreateVehicle(AbstractWorker abstractWorker);

        public bool CheckWorker(PlayerGo client)
        {
            try
            {
