...
namespace Whistler.Jobs.Farm
{
    class FarmManager : Script
    {
        ...
        {
            new FarmZone(0, new Vector3(2081, 4777, 0), new Vector3(1947, 4666, 0), new Vector3(1738, 4846, 0), new Vector3(1855, 5007, 0)),
        ...
        public void onResourceStart()
        {
            try
            {
                foreach (var zone in _zones)
                {
                    ...
                    colShape.OnEntityEnterColShape += (PlayerGo player) => ColShape_OnEntityEnterColShape(player, zone.ID);
                    colShape.OnEntityExitColShape += (PlayerGo player) => ColShape_OnEntityExitColShape(player, zone.ID);
                }
                ...
                Job = new Job
                {
                    ...
                    Levels = new List<JobLevel>
                    {
                        ...
                        new JobLevel(3, 4000, "Fast"),
                        new JobLevel(4, 6000, "Big"),
                        new JobLevel(5, 8000, "Fertilized"),
                        new JobLevel(6, 10000, "BigFertilized"),
                        new JobLevel(7, 12000, "FastFertilized"),
                    },
                ...
                NAPI.Blip.CreateBlip(587, _sellPoint, 1, 52, "Вода", 255, 0, true, 0, 0);
            }
            catch (Exception e)
            {
        ...
        private static void SellHarvest(Player player)
        {
                ...
                new List<DialogUI.ButtonSetting>
                {
                        new DialogUI.ButtonSetting
                        {
                            ...
                            Action = p =>
                            {
                                ...
                                Wallet.MoneyAdd(p.GetCharacter(), price, "Money_SellResources");
                            }
                        },
                        new DialogUI.ButtonSetting
                        {
        ...
        private void ColShape_OnEntityExitColShape(PlayerGo player, int id)
        {
            SubscribeSystem.UnSubscribe(player, id);
            player.TriggerEvent("farm:unloadPoints", id);
        }

        private void ColShape_OnEntityEnterColShape(PlayerGo player, int id)
        {
            SubscribeSystem.Subscribe(player, id);
            player.TriggerEvent("farm:loadPoints", id, JsonConvert.SerializeObject(_plants.Where(item => item.FarmId == id).Select(item => new PlantInLandDto(item))));
        }

        private static SeedingPlaceConfig GetNearestPoint(Player player, int farmId)
        {
        ...
        public static bool OnUseSeed(PlayerGo player, Seed seed)
        {
            ...
            if (point == null)
            {
                Notify.SendError(player, "farmHouse_11");
                return false;
            }
            int countPlant = _plants.Where(item => item.UUID == player.GetCharacter().UUID).Count();
            ...
            {
                Notify.SendError(player, "farmHouse_13".Translate(levelData.CountPits));
                return false;
            }
            if ((int)point.PitType > (int)levelData.BestPits)
            {
                Notify.SendError(player, "farmHouse_14");
                return false;
            }
            if (PlantingSeedOnPoint(player, seed.Name, farmId, point))
            {
                ...
                Notify.SendSuccess(player, "farmHouse_23");
                return true;
            }
            return false;
        }

        public static bool OnUseWateringCan(PlayerGo player, WateringCan watering)
        {
        ...
        public static bool OnUseFertilizer(PlayerGo player, Fertilizer fertilizer)
        {
        ...
        public static bool OnUseHarvesting(PlayerGo player, Other foodBox)
        {
        ...
        #region Farm Moving
        private static bool PlantingSeedOnPoint(PlayerGo player, ItemNames seed, int farmId, SeedingPlaceConfig point)
        {
            ...
            PlantInLand plant = PlantInLand.CreatePlant(farmId, point.ID, player.GetCharacter().UUID, seed);
            if (plant == null)
            {
        ...
        private static bool WateringSeedOnPoint(PlayerGo player, int farmId, SeedingPlaceConfig point, WateringCan watering)
        {
        ...
        private static bool FertilizerSeedOnPoint(PlayerGo player, Fertilizer fertilizer, int farmId, SeedingPlaceConfig point)
        {
            var plant = _plants.FirstOrDefault(item => item.FarmId == farmId && item.PointId == point.ID);
            if (plant == null)
            {
        ...
        private static bool HarvestingFoodOnPoint(PlayerGo player, int farmId, SeedingPlaceConfig point)
        {
            var plant = _plants.FirstOrDefault(item => item.FarmId == farmId && item.PointId == point.ID);
            if (plant == null)
            {
            ...
            {
                if (plant.UUID != player.GetCharacter().UUID)
                {
                    return false;
                }
