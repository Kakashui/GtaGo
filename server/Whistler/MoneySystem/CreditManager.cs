...
namespace Whistler.MoneySystem
{
    class CreditManager
    {
        public static void CreateCredit(Player player, int amount, int days, CreditTypePayment typePayment, int pledgeId, PropertyType pledgeType)
        {
            ...
            Wallet.MoneyAdd(character.BankModel, amount, "Model_CreateCredit");
        ...
        public static void PayCredit(Player player, int id, int amount)
        {
            var credit = GetCreditModel(id);
            if (credit != null)
            {
                amount = Math.Min(credit.GetMaxPay(), amount);
                if (Wallet.MoneySub(player.GetBankAccount(), amount, "Money_PayCredit".Translate(credit.ID)))
                {
        ...
        public static List<CreditDTO> GetCreditDTOs(Player player)
        {
            var credits = EFCore.DBManager.GlobalContext.Credits.Where(item => item.UUID == player.GetCharacter().UUID && !item.ClosedCredit);
            return credits.ToList().Where(item => !item.ClosedCredit).Select(item => new CreditDTO(item)).ToList();
        }
        public static void CalculateCreditParams(Player player, int amount, int days, CreditTypePayment typePayment, bool isPledge)
        {
        ...
        private static void UpdateCredit(Player player, CreditModel credit)
        {
