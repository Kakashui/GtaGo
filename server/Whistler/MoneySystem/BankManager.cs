    ...
    class BankManager : Script
    {
        ...
        public static void SaveBankTransact(IBankAccount from, IBankAccount to, int amount, int tax, string comment)
        {
            try
            {
                ...
                WhistlerTask.Run(() =>
                {
                    (from as CheckingAccount)?.SendTransaction(transaction);
                    (to as CheckingAccount)?.SendTransaction(transaction);
                });
                using (var context = EFCore.DBManager.TemporaryContext)
                {
            ...
            {
                WhistlerTask.Run(() => _logger.WriteError($"SaveBankTransact: {e.ToString()}"));
            }
        }

        public static List<TransactionDTO> GetTransactionHistory(IBankAccount bank)
        {
            try
            {
        ...
        public static void PayPhones()
        {
            EFCore.DBManager.GlobalContext.CheckingAccounts
                ...
                .ForEach(item =>
                {
                    Wallet.MoneySub(item, BankSettings.PhonePayed, null);
                });
        }

        [Command("fixdoubleaccount")]
        public static void FixDoubleAccount(Player player)
        {
            if (player.GetCharacter().AdminLVL < 10) return;
