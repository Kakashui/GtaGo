...
namespace Whistler.MoneySystem
{
    class ATM : Script
    {
        ...
        public static List<Vector3> ATMs = new List<Vector3>
        {
            ...
            new Vector3(237.3561, 217.8394, 105.1667), // 58 gov
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                {
                    if (i != 58) NAPI.Blip.CreateBlip(434, ATMs[i], 0.35f, 11, "Банкомат", shortRange: true, dimension: 0);
        ...
        public static void CreateBankPoint(Player player)
        {
        ...
        public static void CreateBankPoint(Player player, int id)
        {
        ...
        {
            var bankDto = new BankDTO(player);
            List<DepositDTO> depositsDto = DepositManager.GetDepositDTOs(player);
            var business = player.GetBusiness();
            var businessDto = business != null ? new BankBusinessDTO(business, player.GetAccount().IsPrimeActive()) : null;
            var creditDto = new BankCreditDTO(player);
            var family = player.GetFamily();
            var familyDto = family != null ? new BankOrganizationDTO(family, family.IsLeader(player)) : null;
            var fraction = player.GetFraction();
            var fractionDto = fraction != null ? new BankOrganizationDTO(fraction, fraction.IsLeaderOrSub(player)) : null;

            player.TriggerEvent("bankMenu:open", 
        ...
        public static void BankRefillMoney(Player player, int amount, string type, int depositId)
        {
            ...
            switch (type)
            {
                ...
                case "family":
                    ...
                    break;
                case "personal":
                    if (Wallet.TransferMoney(acc, player.GetBankAccount(), amount, 0, "Money_AtmIn"))
                        Notify.SendSuccess(player, "bank:menu:230");
                    else
                        Notify.SendSuccess(player, "bank:menu:218");
                    break;
                case "business":
        ...
        public static void BankWithdrawMoney(Player player, int amount, string type, int depositId)
        {
            var acc = player.GetCharacter();
            switch (type)
            {
                ...
                case "family":
                    ...
                    break;
                case "personal":
                    if (Wallet.TransferMoney(player.GetBankAccount(), acc, amount, 0, "Money_AtmOut"))
        ...
        public static void BankTransferMoney(Player player, string targetNumber, int amount, string comment, string reason)
        {
        ...
        public static void BankPayHouseTax(Player player, int amount, bool personalHouse)
        {
            if (personalHouse)
            {
                var house = HouseManager.GetHouse(player, true);
                if (MoneyManager.PayHouseTax(player, player.GetBankAccount(), amount, house))
                    player.TriggerCefEvent("bank/updateHomeBalance", house.BankModel.IMoneyBalance);
            }
            else
            {
                var famHouse = HouseManager.GetHouseFamily(player);
                if (MoneyManager.PayHouseTax(player, player.GetFamily(), amount, famHouse))
                    player.TriggerCefEvent("bank/family/updateFamilyHomeBalance", famHouse.BankModel.IMoneyBalance);
            }
        }
        [RemoteEvent("bank:payBizTax")]
        public static void BankPayBusinessTax(Player player, int amount)
        {
            if (MoneyManager.PayBusinessTax(player, amount))
                player.TriggerCefEvent("bank/business/updateBizTaxBalance", player.GetBusiness().BankNalogModel.IMoneyBalance);
        }
        [RemoteEvent("bank:openDeposit")]
        public static void BankCreateDeposit(Player player, int typeId, int amount, int days) => DepositManager.CreateDeposit(player, typeId, amount, days);
        
        [RemoteEvent("bank:closeDeposit")]
        public static void BankCreateDeposit(Player player, int id) => DepositManager.CloseDeposit(player, id);

        [RemoteEvent("bank:checkStatusProperty")]
        public static void BankCheckStatusProperty(Player player)
        {
        }
        [RemoteEvent("bank:calcCredit")]
        public static void BankCalculateCredit(Player player, int amount, int days, string typePayment, bool isPledge)
        {
        ...
        public static void BankCreateCredit(Player player, int amount, int days, string typePayment, string pledgedType, int pledgedId)
        {
        ...
        public static void BankPayCredit(Player player, int id, int amount) => CreditManager.PayCredit(player, id, amount);

        [RemoteEvent("bank:payPenalty")]
        public static void BankPayPenalty(Player player, int id)
        {
            if (id == -1)
        ...
        public static void BankPayPhone(Player player, int amount)
        {
            var acc = player.GetCharacter().PhoneTemporary.GetPhoneBankAccount();
            if (Wallet.TransferMoney(player.GetBankAccount(), acc, amount, 0, "Money_PhonePay"))
        ...
        public static void BankPayBonus(Player player, int uuid, int amount, string type)
        {
            switch (type)
            {
                case "fraction":
                    ...
                    {
                        Notify.SendSuccess(player, "bank:menu:219");
                        return;
                    }
                    if (!fraction.Members.ContainsKey(uuid))
                    {
                        Notify.SendSuccess(player, "bank:menu:220");
                        return;
                    }
                    if (!Wallet.TransferMoney(fraction, BankManager.GetAccountByUUID(uuid), amount, 0, "Money_OrgPrize"))
                    {
                        Notify.SendSuccess(player, "bank:menu:221");
                        return;
                    }
                    var payment = new OrgPaymentDTO(uuid, Main.PlayerNames.GetValueOrDefault(uuid), DateTime.Now.ToShortDateString(), amount);
                ...
                case "family":
                    ...
                    if (!Wallet.TransferMoney(family, BankManager.GetAccountByUUID(uuid), amount, 0, "Money_OrgPrize"))
                    {
                        Notify.SendSuccess(player, "bank:menu:223");
                        return;
                    }
