...
namespace Whistler.SDK
{
    public class CharacterData : IMoneyOwner
    {
        ...
        public DateTime ArrestDate { get; set; } = DateTime.UtcNow;
        ...
        public int CountUseMyPromocode { get; set; }
        ...
        public Dictionary<VehicleAccess, Vehicle> TempVehicles = new Dictionary<VehicleAccess, Vehicle>();
        ...
        protected static int _lastUUID = 1000;
        public CharacterData(string firstName, string lastName, int accountId, int customizationId, ClothesDTO clothes)
        {
            ...
            SpawnPos = new Vector3(1228.1294, -3002.344, 9.319266); //новый спавн персонажа

            LifeActivity = new LifeActivity();

            Main.PlayerSlotsInfo.Add(UUID, new SlotInfo(LVL, EXP, FractionID, Money, Customization.Gender, LifeActivity.Hunger.Level, LifeActivity.Thirst.Level, LifeActivity.Rest.Level, LifeActivity.Joy.Level));
            ...
            MyPromocode = PromoCodesService.GeteratePromoCode();
            CountUseMyPromocode = 0;
            ...
            MySQL.Query("INSERT INTO `characters`(`uuid`,`firstname`,`lastname`,`gender`,`health`,`lvl`,`exp`,`money`,`banknew`,`work`,`fraction`,`fractionlvl`,`arrest`,`demorgan`,`wanted`, " +
                "`adminlvl`,`licenses`,`unwarn`,`unmutedate`,`warns`,`onduty`,`lasthour`,`pos`,`createdate`, `friends`, `arrestiligalTime`, `arrestID`, " +
                "`timerMiss`, `courttime`, `mulct`,`partner`,`equipId`,`inventoryId`, `queststage`, `owner`, `customizationid`) " +
                ...
                "@prop18, @prop19, @prop20, @prop21, @prop22, @prop23, @prop24, @prop25, @prop26, @prop27, @prop28, @prop29, @prop30, @prop31, @prop32, @prop33, @prop34, @prop35)",
                ...
                MySQL.ConvertTime(UnmuteDate), Warns, OnDuty, LastHourMin, JsonConvert.SerializeObject(SpawnPos), MySQL.ConvertTime(CreateDate), JsonConvert.SerializeObject(Friends), 
                ArrestiligalTime, ArrestID, MySQL.ConvertTime(TimerMiss), CourtTime, Mulct, Partner, Equip.Id, Inventory.Id, (int)QuestStage, accountId, CustomizationId);
        ...
        public CharacterData(DataRow row)
        {
            ...
            ArrestDate = DateTime.UtcNow.AddMinutes(Convert.ToInt32(row["arrest"]));
            ...
            if (row["pos"].ToString().Contains("NaN"))
            {
                ...
                if (LVL <= 1)
                    SpawnPos = new Vector3(1228.1294, -3002.344, 9.319266); // On newbe spawn  -1037.248 -2731.725 13.75665 //новый спан персонажа
                else
            ...
            MyPromocode = row["mypromocode"].ToString();
            if (string.IsNullOrEmpty(MyPromocode))
            {
                MyPromocode = PromoCodesService.GeteratePromoCode();
                MySQL.QuerySync("UPDATE characters SET mypromocode = @prop0 WHERE uuid = @prop1", MyPromocode, UUID);
            }
            CountUseMyPromocode = Convert.ToInt32(row["countUseMyPromocode"]);
        }
        ...
        public void ClearTempFields()
        {
            ...
            TempVehicles = new Dictionary<VehicleAccess, Vehicle>();
        }
        ...
        public void UpdateFriends(Player player)
        {
            Trigger.ClientEvent(player, "setFriendList", JsonConvert.SerializeObject(Friends));
        }

        public void UpdateReferal(Player player, bool all)
        {
            if (all)
            {
                player.TriggerCefEvent("optionsMenu/updateReferalsData", JsonConvert.SerializeObject(new { key = "total", value = CountUseMyPromocode }));
                player.TriggerCefEvent("optionsMenu/updateReferalsData", JsonConvert.SerializeObject(new { key = "code", value = MyPromocode }));
                }
            else
            {
                player.TriggerCefEvent("optionsMenu/updateReferalsData", JsonConvert.SerializeObject(new { key = "total", value = CountUseMyPromocode }));
        ...
        private bool Change(int amount)
        {
            ...
            Money += amount;
            Trigger.ClientEvent(Player, "UpdateMoney", Money.ToString(), Convert.ToString(amount));
            return true;
        }
        public void SetPartner(Player player, Player target)
        {
            var targetUuid = target.GetCharacter().UUID;
            Partner = targetUuid;
            MySQL.Query("UPDATE characters SET partner = @prop1 WHERE uuid = @prop0", targetUuid, UUID);
            Trigger.ClientEvent(player, "marriage:complete", target.Name.Replace('_', ' '));
        }
        ...
        public static void AddOfflineRespectPoint(int uuid, int value)
        {
            ...
            Main.GetCharacterByUUID(uuid)?.ChangeRespectPoint(value);
        }
    ...
    public class PlayerIconOverHead
    {
        ...
        public void UpdateSharedData(Player player)
        {
            if (Dictionary != "none")
            {
                player.SetSharedData("playerIcon:dict", Dictionary);
                player.SetSharedData("playerIcon:name", Name);
                player.SetSharedData("playerIcon:color", Color);
            }
            else
            {
                player.ResetSharedData("playerIcon:dict");
                player.ResetSharedData("playerIcon:name");
                player.ResetSharedData("playerIcon:color");
            }
        }
    }
}
