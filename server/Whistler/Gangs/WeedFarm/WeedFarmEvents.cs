...
namespace Whistler.Gangs.WeedFarm
{
    class WeedFarmEvents: Script
    {
        ...
        public void OnPlayerDisconnect(PlayerGo player, DisconnectionType type, string reason)
        {
            WeedFarmService.DisconnectHandle(player);
        }

        [RemoteEvent("weedfarm:comp:path:set")]
        public void SetPathComponent(PlayerGo player, double x, double y)
        {
            player.SetData("weedfarm:comp:path", new Vector3(x, y, 0));
        }

        [RemoteEvent("weedfarm:sort:action:request")]
        public void RequestPackAction(PlayerGo player)
        {
        ...
        public void BadFarmDimension(PlayerGo player)
        {
            if (!player.IsLogged()) return;
        ...
        public void CancelSortJob(PlayerGo player)
        {
            if (!player.IsLogged()) return;
        ...
        public void DeliveryBegine(PlayerGo player)
        {
            if (!player.IsLogged()) return;
        ...
        public void DeliveryEnd(PlayerGo player)
        {
            if (!player.IsLogged()) return;
        ...
        public void DeliveryAction(PlayerGo player)
        {
            if (!player.IsLogged()) return;
        ...
        [Command("setwfowner")]
        public void SetWeedFarmOwner(PlayerGo player, int farmId, int gangId)
        {
        ...
        public void StartWeedFarmBattle(PlayerGo player, int farmId)
        {
        ...
        [Command("respfarmcar")]
        public void RespawnFarmCar(PlayerGo player,  int farmId)
        {
            if (!Manager.CanUseCommand(player, "respfarmcar"))
            {
                player.SendError("Core_69");
                return;
            };
