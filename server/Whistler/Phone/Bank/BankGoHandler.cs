...
namespace Whistler.Phone.Bank
{
    class BankGoHandler : Script
    {
        ...
        private static void AcceptTransferMoneyToBankAccount(Player player, string bankTarget, int amount)
        {
        ...
        private static void AcceptPayHouseTax(Player player, int amount)
        {
            if (MoneyManager.PayHouseTax(player, player.GetBankAccount(), amount, HouseManager.GetHouse(player, true)))
            {
                player.TriggerCefAction("smartphone/bankPage/setCompleateTaxPay", true);
            }
        }
        [RemoteEvent("phone::bank::payBusinessTax")]
        private static void AcceptPayBusinessTax(Player player, int amount)
        {
        ...
        private static void AcceptPayPenalty(Player player)
        {
