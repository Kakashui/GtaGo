...
namespace Whistler.Phone.Messenger.Chat
{
    internal class JoinChatByInviteHandler : Script
    {
        ...
        public async Task HandleJoinChatByInvite(Player player, string inviteCode)
        {
            try
            {
                using (var context = DbManager.TemporaryContext)
                {
                    ...
                    var playerAccountId = player.GetCharacter().PhoneTemporary.Account.Id;
                    ...
                    var account = player.GetCharacter().PhoneTemporary.Account;
