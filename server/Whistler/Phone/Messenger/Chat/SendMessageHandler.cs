...
namespace Whistler.Phone.Messenger.Chat
{
    internal class SendMessageHandler : Script
    {
        ...
        public async Task SendMessage(Player player, int chatId, string messageJson)
        {
            try
            {
        ...
        private List<Attachment> GetAttachments(Player player, JObject messageObj)
        {
            var attachments = new List<Attachment>();

            if (messageObj["attachment"] != null)
            {
                var attachmentObj = messageObj["attachment"];
                var type = attachmentObj["type"].ToObject<AttachmentType>();
