...
namespace Whistler.Phone.Messenger.Accounts
{
    internal class CreateAccountHandler : Script
    {
        ...
        public async Task HandleCreateAccount(Player player, string username, string displayname)
        {
            try
            {
                ...
                using (var context = DbManager.TemporaryContext)
                {
                    ...
                    var simcard = character.PhoneTemporary.Simcard;
                    ...
                    character.PhoneTemporary.Account = account;
