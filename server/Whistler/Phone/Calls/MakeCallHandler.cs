...
namespace Whistler.Phone.Calls
{
    ...
    {
        public static Action<Player, int> CallShortNumber;
        private static WhistlerLogger _logger = new WhistlerLogger(typeof(MakeCallHandler));

        [RemoteEvent("phone::calls::call")]
        public async Task MakeCallAsync(PlayerGo player, int targetNumber)
        {
            try
            {
                if (!player.IsLogged())
                ...
                using (var context = DbManager.TemporaryContext)
                {
                    ...
                    if (character.PhoneTemporary.Simcard.Number == targetNumber)
                    {
        ...
        private async Task SendPlayerFailedCall(Player player, int targetNumber, string reason)
        {
            try
            {
                var simcard = player.GetCharacter().PhoneTemporary.Simcard;
