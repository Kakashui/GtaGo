...
namespace Whistler.Phone.Taxi.Service.Models
{
    class TaxiOrderPlayer : TaxiOrderBase
    {
        ...
        {
            Player client = Main.GetPlayerByUUID(PassengerUuid);
            if (client.IsLogged())
                client.TriggerCefEvent("smartphone/taxiPage/taxi_setDriverData", 
                    JsonConvert.SerializeObject(new DriverDataDto(driver.GetCharacter(), driverData)));
        }

        public override bool CompleateOrder()
        {
            var passenger = Main.GetPlayerByUUID(PassengerUuid);
        ...
        {
            base.Destroy();
            Main.GetPlayerByUUID(PassengerUuid)?.TriggerCefAction("smartphone/taxiPage/taxi_goHome", true);
        }
