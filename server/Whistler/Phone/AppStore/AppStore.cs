...
namespace Whistler.Phone.AppStore
{
    public class AppStore : Script
    {
        ...
        private async Task LoadPlayerApps(PlayerGo player, Character character)
        {
            try
            {
            ...
            {
                WhistlerTask.Run(() => _logger.WriteError($"Unhandled exception catched on LoadPlayerApps ({player?.Name}) - " + e.ToString()));
            }
        }

        [RemoteEvent("phone:appStore:install")]
        public void InstallPlayerApplication(Player player, int applicationId)
        {
            try
            {
        ...
        public void RemovePlayerApplication(Player player, int applicationId)
        {
            try
            {
                var character = player.GetCharacter();
