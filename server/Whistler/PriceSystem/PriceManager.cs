...
namespace Whistler.PriceSystem
{
    class PriceManager : Script
    {
        ...
        private static bool AddItem(TypePrice type, string name, int price)
        {
        ...
        public static void OpenPriceMenu(Player player)
        {
            ...
            player.TriggerEvent("priceMenu:open", JsonConvert.SerializeObject(_prices), cats);
        }
        
        [RemoteEvent("priceMenu:addItem")]
        public static void PriceMenuAddItem(Player player, string name, int price)
        {
            if (!Group.CanUseAdminCommand(player, "openpricemenu")) return;
        ...
        public static void PriceMenuChangePrice(Player player, string name, int price)
        {
            if (!Group.CanUseAdminCommand(player, "openpricemenu")) return;
            var split = name.Split("::");
            if (split.Length != 2 || !Enum.IsDefined(typeof(TypePrice), split[0]) || price <= 0)
        ...
        public static void PriceMenuDeleteItem(Player player, string name)
        {
            if (!Group.CanUseAdminCommand(player, "openpricemenu")) return;
            if (DeleteItem(name))
            {
        ...
        private static void UpdateItemInMenu(Player player, string name, int price)
        {
