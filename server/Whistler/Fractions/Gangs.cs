...
namespace Whistler.Fractions
{
    class Gangs : Script
    {
        ...
        private static void GiveArmor(Player player)
        {
            if (!Wallet.MoneySub(player.GetCharacter(), _armorPrice, "Money_PedArmor"))
            {
        ...
        private static void GiveNarko(Player player)
        {
            if (!Wallet.MoneySub(player.GetCharacter(), _narkoPrice, "Money_PedNarko"))
            {
                Notify.SendError(player, "frac_556");
                return;
            }
