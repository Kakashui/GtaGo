...
namespace Whistler.Fractions.GOV
{
    class Government : Script
    {
        ...
        public Government()
        {
            ...
            {
                leftLift.AddFloor(model.Name, model.Position, model.Rotation, dimension: model.Dimension, marker: false, exitPredicate: (Player p) => CheckExitAccess(p, model.Access), input: false);
            });
            ...
            {
                rightLift.AddFloor(model.Name, model.Position, model.Rotation, dimension: model.Dimension, marker: false, exitPredicate: (Player p) => CheckExitAccess(p, model.Access), input: false);
            });
        ...
        public static void SendGovernmentNotify(Player player, string title, string message)
        {
            player.TriggerEvent("government:sendNotify", title, message);
        }
        ...
        private static void LoadTopPhilanthropists(Player player, int countBest)
        {
            if (countBest <= 0)
                return;
            ...
            player.TriggerEvent("government:loadPhilanthropistsList", JsonConvert.SerializeObject(philanthropists));
        }

        private static void interactPressedClothes(Player player)
        {
            if (player.GetCharacter().FractionID == 17)
                SkinManager.OpenSkinMenu(player);
            else
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Frac_522", 3000);
        }

        private static bool CheckExitAccess(Player player, int access)
        {
            return player.GetCharacter().FractionID == 17 || player.IsAdmin();
        }

        public static void OpenGovernmentMenu(Whistler.Entities.PlayerGo player)
        {
            if (!player.IsLogged())
                return;
            List<SocialStatusDTO> socialStatus = new List<SocialStatusDTO>
            {
                new SocialStatusDTO()
                {
                    ...
                    {
                        "cityHall_1",
                        player.GetCharacter().GetPartnerName()
                    }
                },
                new SocialStatusDTO()
                {
            ...
            player.TriggerEvent("government:openVoteMenu",
                player.GetCharacter().UUID,
                JsonConvert.SerializeObject(new 
                { 
                    voteDone = DateTime.Now > Consignments.FinishVoteDate, 
                    currentVote = player.GetCharacter().LastVote, 
        ...
        public static void RemoteEvent_SendVote(Player player, int vote)
        {
            if (!player.IsLogged())
                return;
            if (Consignments.ConsignmentVotes.ContainsKey(vote))
            {
                MySQL.Query("UPDATE `characters` SET `lastvote` = @prop0 WHERE `uuid` = @prop1", vote, player.GetCharacter().UUID);
                player.GetCharacter().LastVote = vote;
                player.TriggerEvent("government:setVote", player.GetCharacter().LastVote);
            }
        }
        [RemoteEvent("government:createComplaint")]
        public static void RemoteEvent_CreateComplaint(Player player, int fraction, string name, string text)
        {
            if (!player.IsLogged())
                return;
            ...
            _complaints.Add(new Complaint(player.GetCharacter().UUID, fraction, target.Key, text));
            SendGovernmentNotify(player, "cityHall_7", "cityHall_8");
        }
        [RemoteEvent("government:createRecord")]
        public static void RemoteEvent_CreateRecord(Player player, string employee, string text)
        {
            if (!player.IsLogged())
                return;
        ...
        public static void RemoteEvent_ChangeName(Player player, string newName, int paymentType)
        {
            if (!player.IsLogged())
                return;
            ...
            var result = Character.ChangeName(player.GetCharacter().FullName, newName);
            switch (result)
            {
        ...
        public static void RemoteEvent_CreateFamily(PlayerGo player, string familyName, int paymentType, string leaders, string nation)
        {
            if (!player.IsLogged())
                return;
            ...
            if (FamilyManager.CreateFamily(player, familyName, _createFamilyPrice, (PaymentsType)paymentType, 
                (Player player, bool success, string message) => 
                {
                    if (success)
                ...
                }))
            {
                ...
                {
                    PlayerGo target = NAPI.Player.GetPlayerFromName(item) as PlayerGo;
                    if (!target.IsLogged())
                        continue;
        ...
        public static void RemoteEvent_DonateToGov(PlayerGo player, int amount, int type, bool menuGov)
        {
            if (!player.IsLogged())
                return;
            if (Wallet.MoneySub(player.GetCharacter(), amount, "Money_Philanthrop"))
            {
        ...
        public static void RemoteEvent_BuyLicense(Player player, int typeLic, int typePay)
        {
            if (!player.IsLogged())
                return;
            if (typeLic < 9)
