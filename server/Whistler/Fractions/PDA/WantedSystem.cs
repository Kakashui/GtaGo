...
namespace Whistler.Fractions.PDA
{
    ...
    {
        public static void UpdatePlayerWantedLevel(Player player, int level)
        {
            if (!player.IsLogged())
                return;
            Trigger.ClientEvent(player, "setWanted", level);
        }
        public static void SetPlayerWantedLevel(Player player, Player whoGive, int lvl, string reason)
        {
            if (!player.IsLogged())
                return;
            ...
            else
            {
                ...
                {
                    character.WantedLVL = new WantedLevel(lvl, whoGive.Name, DateTime.Now, reason);
                }    
                else
                {
                    character.WantedLVL.Level = lvl;
                    character.WantedLVL.Reason = reason;
                    character.WantedLVL.WhoGive = whoGive.Name;
                }
            }
        ...
        {
            var list = Main.GetPlayersGoListByPredicate((player) => player.Character.WantedLVL != null);
            return JsonConvert.SerializeObject(list.Select(item => new { name = item.Name, passport = item.Character.UUID, sex = item.Character.Customization.Gender, number = item.GetPhone()?.Simcard.Number, wantedLevel = item.Character.WantedLVL.Level, licenses = GetLicenses(item.Character.Licenses)}));
        }
