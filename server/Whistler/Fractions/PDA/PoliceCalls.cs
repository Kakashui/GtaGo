...
namespace Whistler.Fractions.PDA
{
    class PoliceCalls
    {
        ...
        public static void CreatePoliceCall(Player player, int code)
        {
            ...
            int id = _counter++;
            PoliceCall call = new PoliceCall(player, id, code, player.Name, player.GetCharacter().UUID, player.Position);
            ...
            string message = code == 999 ? $"Frac_547".Translate(player.GetCharacter().UUID) : "Frac_459".Translate(player.GetCharacter().UUID, $"Code {code}");
            Chat.SendFractionMessage(7, message, false);
            Chat.SendFractionMessage(9, message, false);
        }

        public static void DeletePoliceCallOfPlayer(Player player, bool accept = false)
        {
            var call = _policeCalls.FirstOrDefault(item => item.Value.CallerUUID == player.GetCharacter().UUID);
            if (call.Value != null)
            {
        ...
        public static void AcceptCall(Player player, int callId)
        {
        ...
        public static void SubHelperForAllCalls(Player player, bool trigger)
        {
