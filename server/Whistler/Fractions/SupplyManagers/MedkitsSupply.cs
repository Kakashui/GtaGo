...
namespace Whistler.Fractions.SupplyManagers
{
    public class MedkitsSupply : Script
    {
        ...
        private static Vehicle _plane = null;
        ...
        private void PlayerLandPlane(ColShape colShape, Player player)
        {
            if (!player.HasData("SUPPLYMEDS") || !player.IsInVehicle) 
                return;
            VehicleGo vehGo = player.Vehicle.GetVehicleGo();
            if (vehGo.Data.OwnerType != OwnerType.Temporary || (vehGo.Data as TemporaryVehicle).Access != VehicleAccess.MedKits)
                return;

            Notify.Send(player, NotifyType.Alert, NotifyPosition.Bottom, "msply:1", 3000);

            var plane = player.Vehicle;
            ...
            WhistlerTask.Run(() =>
            {
                ...
                var inPlaneTrunkPosition = plane.Position + plane.Rotation.GetDirectionFromRotation().Multiply(-1).Multiply(4) - new Vector3(0, 0, 0.45);

                _interactShape = InteractShape.Create(inPlaneTrunkPosition, 1, 2)
                    .AddDefaultMarker()
                    .AddInteraction(GetMedkits);

                VehicleStreaming.SetDoorState(plane, DoorID.DoorTrunk, DoorState.DoorOpen);
            }, 3000);
        }

        private void GetMedkits(PlayerGo player)
        {
        ...
        public void EndMedkitsSupply(Player player)
        {
            try
            {
        ...
        [Command("supplymed")]
        public void StartMedkitsSupply(Player player)
        {
            try
            {
                ...
                player.SetData("SUPPLYMEDS", true);
