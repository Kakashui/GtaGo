...
namespace Whistler.Fractions.AttackArmy.Models
{
    class AttackMembers
    {
        ...
        public void Payment()
        {
            ...
            {
                Payment(item, Constants.CrimeStationBrokenMoney, "elstation:prize:1".Translate(Constants.CrimeStationBrokenMoney), "Money_AttackNGbroken");
            }
            foreach (var item in _crimeLoadItems)
            {
                Payment(item, Constants.CrimeLoadItemsMoney, "elstation:prize:2".Translate(Constants.CrimeLoadItemsMoney), "Money_AttackNGload");
            }
            foreach (var item in _armyRepairStation)
            {
                Payment(item, Constants.ArmyRepairStationMoney, "elstation:prize:3".Translate(Constants.ArmyRepairStationMoney), "Money_AttackNGrepair");
            }
        }
        private void Payment(int member, int money, string message, string reason)
        {
            ...
            var player = Main.GetPlayerByUUID(member);
            if (player.IsLogged())
            {
