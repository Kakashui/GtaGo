...
namespace Whistler.Fractions.AttackArmy
{
    class AttackManager : Script
    {
        ...
        public static bool CheckLoadItems(Player player)
        {
        ...
        private static void SetGlobalStatus(PowerStationStatus status)
        {
            ...
            foreach (var station in _stations)
            {
                ...
                if (station.InteractedPlayer.IsLogged())
                {
                    int member = station.InteractedPlayer?.GetCharacter()?.UUID ?? 0;
                    if (member > 0)
        ...
        public static void AddLoaderItems(Player player)
        {
            if (!player.IsLogged())
                return;
            int member = player?.GetCharacter()?.UUID ?? 0;
            if (member > 0)
        ...
        public static bool CheckAccess(Player player, bool isGang, bool onlyLeader, bool notify = true)
        {
            if (isGang)
            {
                if (onlyLeader)
