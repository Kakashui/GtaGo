...
namespace Whistler.Fractions
{
    class Fbi : Script
    {
        ...
        public static void callFibs(Player player)
        {
            try
            {
                ...
                if (DateTime.Now > access)
                {
                    ...
                    player.SetData("playerSendFraction", DateTime.Now.AddMinutes(1));
                }
        ...
        private static void ChangePos49(Player player)
        {
            player.ChangePosition(fbiCheckpoints[3] + new Vector3(0, 0, 1.12));
        }

        private static void ChangePos53(Player player)
        {
            player.ChangePosition(fbiCheckpoints[2] + new Vector3(0, 0, 1.12));
        }

        [Command("accesslift")]
        public static void Command_GiveAccessToLift(Player player, int id)
        {
            ...
            target.SetData("accessFibLiftTime", DateTime.Now.AddMinutes(1));
            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Frac_543".Translate(target.GetCharacter().UUID), 3000);
        }
    }
}