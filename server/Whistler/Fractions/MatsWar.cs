...
namespace Whistler.Fractions
{
    ...
    class MatsWar : Script
    {
        ...
        public static void SpawnMatCar()
        {
            WhistlerTask.Run(() =>
            {
                ...
                if (_numberCar <= _maxCars)
                {
                    ...
                    Vehicle vehicle = VehicleManager.CreateTemporaryVehicle("landstalker2", _carPosition[point], _carRotation[point], $"MATWAR{_numberCar}", VehicleAccess.MatWars);
                    vehicle.SetMod(55, 1);
                    vehicle.SetMod(53, 1);
                    var vehInv = InventoryService.GetById(vehicle.GetVehicleGo().Data.InventoryId);
        ...
        public static void DestroyCar(Vehicle vehicle)
        {
