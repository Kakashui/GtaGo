...
namespace Whistler.Fractions
{
    class GangsCapture : Script
    {
        ...
        private static Dictionary<int, List<Player>> _playerInTeamMenu = new Dictionary<int, List<Player>>
        {
            { 1, new List<Player>() },
            { 2, new List<Player>() },
            { 3, new List<Player>() },
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                foreach (var gangpoint in gangPoints.Values)
                {
                    ...
                    colShape.OnEntityEnterColShape += (PlayerGo player) => onPlayerEnterGangPoint(player, gangpoint.ID);
                    colShape.OnEntityExitColShape += (PlayerGo player) => onPlayerExitGangPoint(player, gangpoint.ID);
                }
        ...
        {
            var fractionId = player.GetCharacter().FractionID;
        ...
        [Command("fixcapt")]
        public static void CMD_FixCapt(Player player)
        {
            try
            {
        ...
        public static void CMD_OpenCaptTeamMenu(Player player)
        {
            ...
            try
            {
                ...
                int enemyFracID = 0;
                var point = WaitingCapt.FirstOrDefault(item => gangPoints[item.Key].GangOwner == player.GetCharacter().FractionID);
                ...
                //Отправляем данные
                Trigger.ClientEvent(player, "capt:openTeamMenu", JsonConvert.SerializeObject(listMembers), fraction.Id, enemyFracID, attackStatus);
        ...
        public static void Event_InviteMember(Player player, string name)
        {
            try
            {
                if (!player.IsLogged())
                ...
                Player target = Main.GetPlayerByName(name);
                if (target == null)
                    return;
                if (target.GetCharacter().FractionID != fracID)
                    return;
                SetMemberInGangTeam(target, 1);
                target.TriggerEvent("openDialog", "INVITECAPT", "Frac_525".Translate());
            }
            catch (Exception e)
            {
        ...
        public static void Event_KickMember(Player player, string name)
        {
            try
            {
                if (!player.IsLogged())
        ...
        public static void Event_ExitTeamMenu(Player player)
        {
            try
            {
                if (!player.IsLogged())
        ...
        public static void Event_AttackToEnemy(Player player)
        {
            CMD_startCapture(player);
        }

        [RemoteEvent("capt:deffendRegion")]
        public static void Event_DeffendRegion(Player player)
        {
            CMD_DefendersReady(player);
        }

        public static void SetMemberInGangTeam(Player player, int state)
        {
            try
            {
                if (!player.IsLogged())
                ...
                if (fraction == null || !_playerInTeamMenu.ContainsKey(fraction.Id))
                    return;
                if (state == 2)
                    player.SetData("CAPTTEAM", true);
                else
                    player.ResetData("CAPTTEAM");
                Trigger.ClientEventToPlayers(_playerInTeamMenu[fraction.Id].ToArray(), "capt:setMember", player.Name, fraction.GetRank(player).RankName, state);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_startCapture(Player player)
        {
            try
            {
                ...
                int attackFrack = player.GetCharacter().FractionID;
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Frac_530", 6000);
                    return;
                }
                ...
                NAPI.Pools.GetAllPlayers().ForEach(p =>
                {
                    if(p.IsLogged())
                    {
                        if(p.GetCharacter().FractionID == attackFrack)
                            CaptureUI.SendUntilCaptureTimer(p, waitingConfig.WaitingTime, 0, "timer_to_capture_0");
                        else if(p.GetCharacter().FractionID == region.GangOwner)
                        {
                            CaptureUI.SendUntilCaptureTimer(p, waitingConfig.WaitingTime, 0, "timer_to_capture_0");
                            if (Manager.CanUseCommand(p, "capture", false))
                                p.TriggerEvent("mmenu:frac:capt:attack");
                        }
                    }
                });
        ...
        public static void CMD_DefendersReady(Player player)
        {
            try
            {
                var point = WaitingCapt.FirstOrDefault(item => gangPoints[item.Key].GangOwner == player.GetCharacter().FractionID);
                ...
                Dictionary<Player, bool> attackers = new Dictionary<Player, bool>();
                Dictionary<Player, bool> defenders = new Dictionary<Player, bool>();
        ...
        public static void StartGangCapture(Dictionary<Player, bool> attackers, Dictionary<Player, bool> defenders, int idGangPoint)
        {
            try
            {
                ...
                List<PlayerGo> playerInPoint = Main.GetPlayersGoListByPredicate((player) => player.HasData("GANGPOINT") && player.GetData<int>("GANGPOINT") == idGangPoint && (player.Character.FractionID == attackFrack || player.Character.FractionID == region.GangOwner));
                ...
                {
                    if (p.Value)
                        p.Key.Dimension = CaptList[idGangPoint].Dimension;
                }
                ...
                var gangsters = NAPI.Pools.GetAllPlayers()
                    .Where(p => p.IsLogged())
                    .Where(p => p.GetCharacter().FractionID == attackFrack || p.GetCharacter().FractionID == region.GangOwner);
        ...
        public static void EndWaitCapture(int gangPointID, bool defendIsLost = true)
        {
            try
            {
                ...
                foreach (var player in Manager.GetFraction(attackersID).OnlineMembers.Values)
                {
                    if (player.IsLogged())
                    {
                        Wallet.MoneyAdd(player.GetCharacter(), _winCaptureMoney, "Money_WinCapture");
                    }
                }
        ...
        public static void EndGangCapture(int idPoint)
        {
            try
            {
                ...
                {
                    if (p.Key.Dimension == capt.Dimension)
                        p.Key.Dimension = 0;
                }
        ...
        public static void ShowHud(Player player, bool show, bool resetKillLog, int firstTeam = 0, int secondTeam = 0, int captTime = 0, int firstTeamCnt = 0, int secondTeamCnt = 0)
        {
            try
            {
                if (show)
        ...
        private static void onPlayerEnterGangPoint(PlayerGo player, int idPoint)
        {
            if (!player.IsLogged()) return;
            player.SetData("GANGPOINT", idPoint);

            if (CaptList.ContainsKey(idPoint) && (player.GetCharacter().FractionID == CaptList[idPoint].AttackersFracId || player.GetCharacter().FractionID == CaptList[idPoint].DefendersFracId))
            {
                ...
                if (CaptList[idPoint].Attackers.ContainsKey(player))
                {
                    ...
                    if (player.Dimension != CaptList[idPoint].Dimension)
                        player.Dimension = CaptList[idPoint].Dimension;
                }
        ...
        private static void onPlayerExitGangPoint(PlayerGo player, int idPoint)
        {
            if (!player.IsLogged()) return;
        ...
        public static bool Event_PlayerDeath(Player player, PlayerGo killer, uint weapon)
        {
            try
            {
                if (CaptList.Count == 0)
                    return false;
                var capt = CaptList.FirstOrDefault(item => item.Value.Attackers.ContainsKey(player) || item.Value.Defenders.ContainsKey(player));
                if (capt.Value == null)
                    return false;

                if (capt.Value.Attackers.ContainsKey(player))
                    capt.Value.Attackers.Remove(player);
                else if (capt.Value.Defenders.ContainsKey(player))
                    capt.Value.Defenders.Remove(player);
        ...
        public static void LoadBlips(Player player)
        {
            try
            {
                ...
                foreach (var zone in CaptList)
                    Trigger.ClientEvent(player, "setZoneFlash", zone.Key, true, gangPointsColor[zone.Value.DefendersFracId]);
                foreach (var zone in WaitingCapt)
                    Trigger.ClientEvent(player, "setZoneFlash", zone.Key, true, gangPointsColor[gangPoints[zone.Key].GangOwner]);
            }
            catch (Exception ex)
            {
        ...
        public class CaptConfig
        {
            ...
            public Dictionary<Player, bool> Defenders { get; set; }
            public Dictionary<Player, bool> Attackers { get; set; }
            public List<PlayerGo> InsideZone { get; set; }
            ...
            public CaptConfig(Dictionary<Player, bool> defenders, Dictionary<Player, bool> attackers, List<PlayerGo> insideZone, int defendersFracId, int attackersFracId, string timerCapt, int timeForCapt)
            {
                Defenders = defenders;
                Attackers = attackers;
