...
namespace Whistler.Fractions
{
    class LSNewsManager : Script
    {
        ...
        public void RemoteEvent_OpenLSNews(Player player)
        {
            try
            {
                if (!player.IsLogged())
                    return;
                if (player.GetCharacter().FractionID != 15)
                    return;
                ...
                Trigger.ClientEvent(player, "lsnews::open");
            }
            catch (Exception e)
            {
                WhistlerTask.Run(() => _logger.WriteError($"RemoteEvent_OpenLSNews: {e.ToString()}"));
            }
        }

        [RemoteEvent("lsnews::compleateAdvert")]
        public void RemoteEvent_CompleateAdvert(Player player, int id, string text, string image)
        {
            try
            {
                if (!player.IsLogged())
            ...
            {
                WhistlerTask.Run(() => _logger.WriteError($"RemoteEvent_CompleateAdvert: {e.ToString()}"));
            }
        }

        [RemoteEvent("lsnews::canceledAdvert")]
        public void RemoteEvent_CanceledAdvert(Player player, int id)
        {
            try
            {
                if (!player.IsLogged())
            ...
            {
                WhistlerTask.Run(() => _logger.WriteError($"RemoteEvent_CanceledAdvert: {e.ToString()}"));
            }
        }
        ...
        private static void LoadStatistics(Player player)
        {
            var stats = GetStatistics();
