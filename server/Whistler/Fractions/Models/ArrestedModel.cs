...
namespace Whistler.Fractions.Models
{
    class ArrestedModel
    {
        ...
        public ArrestedModel(Player player, Player officer, string reason)
        {
            Uuid = player.GetCharacter().UUID;
            DetainedUUID = officer.GetCharacter().UUID;
            Licenses = WantedSystem.GetLicenses(player.GetCharacter().Licenses);
            Gender = player.GetGender();
            NumberPhone = player.GetPhone()?.Simcard.Number ?? -1;
            WantedLevel = player.GetCharacter().WantedLVL.Level;
