...
namespace Whistler.Core
{
    ...
    {
        public static Action<Player, Business> PlayerEnterUnloadShape;
        ...
        public int BizTax
        {
            get
            {
                return Convert.ToInt32(SellPrice * MoneyConstants.PayTaxCoeffForHour);
            }
        }
        [JsonIgnore]
        ...
        public Business(int id, int sellPrice, int type, List<Product> products, Vector3 enterPoint)
        {
            ...
            CreateEnterColshape();
            CreateBlip();
            MySQL.Query($"INSERT INTO businesses (id, owneruuid, sellprice, type, products, enterpoint, unloadpoint, banknew, bankacc, mafia, orders, name, peds, daywithoutproducts) " +
                $"VALUES (@prop0, @prop1, @prop2, @prop3, @prop4, @prop5, @prop6, @prop7, @prop8, @prop9, @prop10, @prop11, @Prop12, @Prop13)",
                ID,
                OwnerID,
                SellPrice,
                Type,
        ...
        public Business(DataRow row)
        {
            ...
            {
                BankNalog = BankManager.CreateAccount(TypeBankAccount.Business).ID;
            ...
            TakeoverDate = row["takeoverdate"] == DBNull.Value ? DateTime.Now : Convert.ToDateTime(row["takeoverdate"]);
            DayWithoutProducts = row["daywithoutproducts"] == DBNull.Value ? DateTime.Now : Convert.ToDateTime(row["daywithoutproducts"]);

            Orders = JsonConvert.DeserializeObject<List<Order>>(row["orders"].ToString());
        ...
        public void Save()
        {
            MySQL.Query("UPDATE `businesses` SET sellprice = @prop1, products = @prop2, family = @prop3, orders = @prop4, name = @prop5," +
                "peds = @prop6, colshapeRange = @prop7, enterpoint = @prop8, blipPosition = @prop9, dimension = @prop10, takeoverdate = @prop11 WHERE id = @prop0",
                ...
                this.Dimension, MySQL.ConvertTime(TakeoverDate));
        }
        ...
        public void CreateBlip()
        {
                ...
                case 1:
                    {
                        ...
                        {
                            prodsCount += prod.Lefts;
                        }
                        if (prodsCount > 50)
                        {
                            bColor = 24;
                            bName = TypeModel.TypeName + " ";
                        }
                        blipScale = 1;
                        bChanged = true;
                        break;
            ...
            if (bChanged)
            {
                blip = NAPI.Blip.CreateBlip(Convert.ToUInt32(TypeModel.BlipType), _blipPosition, blipScale,
                                Convert.ToByte(bColor), Main.StringToU16(bName), 255, 0, true);
            }
            else
            {
                blip = NAPI.Blip.CreateBlip(Convert.ToUInt32(TypeModel.BlipType), _blipPosition, blipScale,
                                Convert.ToByte(TypeModel.BlipColor), Main.StringToU16(TypeModel.TypeName), 255, 0, true);
            }
        ...
        private InteractShape CreateEnterShape(Vector3 position)
        {
            ...
            if (Type != 19) 
                interactshape.AddInteraction(BusinessManager.interactionPressed, "interact_15");
            interactshape.AddInteraction(BusinessManager.GetBusinessInformation, "interact_16", Key.VK_I);
        ...
        public void ChangeEnterPoint(Vector3 newPoint)
        {
            ...
            var admins = NAPI.Pools.GetAllPlayers().Where(p => p != null && p.IsLogged() && p.GetCharacter().AdminLVL > 0).ToArray();
            Trigger.ClientEventToPlayers(admins, "businesses:setMarker", this.ID, newPoint, this.ColshapeRange);
        }
        public void SetOwner(int uuid)
        {
            var oldOwner = Main.GetPlayerByUUID(OwnerID);
            ...
            if (oldOwnerID > 0)
            {
                var bank = BankManager.GetAccountByUUID(oldOwnerID);
                if (bank != null)
                {
                    Wallet.TransferMoney(BankAccountModel, bank, (int)BankAccountModel.IMoneyBalance, 0, "Money_ToOldOwner");
                }
            }
        ...
        public dynamic GetFamilyData()
        {
            return new
            {
                ...
                {
                    new { desc = "famdata:date", value = TakeoverDate.ToString("d") },
                    new { desc = "famdata:profit", value = GetFamilyTax() },
                },
                income = GetFamilyTax(),
                owner = GetOwnerName(),
        ...
        public bool TakeBusinessFromOwner(int price, string reason, string message = null)
        {
            if (Main.PlayerNames.ContainsKey(OwnerID))
            {
                ...
                if (message != null)
                {
                    Player player = Main.GetPlayerByUUID(OwnerID);
                    if (player != null)
                    {
            ...
            if (product != null)
            {
                ...
                if (productType != null)
                {
                    if (productType.MaxMinType == "%")
                        return new ProductPrice(productType.OrderPrice, (int)(productType.OrderPrice * (product.Price / 100.0)), product.Lefts);
                    else
        ...
        public ProductPrice GetPriceByProductId(int id)
        {
            if (Products.Count > id)
                return GetPriceByProductName(Products[id].Name);
            return null;
        }
