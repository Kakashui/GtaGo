...
namespace Whistler.Core
{
    ...
    {
        public static event Action<Player> PlayerBuyTuning;
        ...
        public static void LsCustomOpen(Player player, bool IsAdmin = false)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Biz_124", 3000);
                    return;
                }
                ...
                if (IsAdmin)
                {
                    player.GetCharacter().BusinessInsideId = -2;
                }
                else
                {
                ...
                string vehicleModel = vehGo.Data.ModelName;
                if (VehicleModels.VehicleModelNames.ContainsKey(player.Vehicle.Model))
                    cntTurboStage = 4;
                modelPrice = PriceManager.GetPriceInDollars(TypePrice.Car, vehGo.Data.ModelName, 5000000);
                Trigger.ClientEvent(player, "lsCustom:openTun", pricePart, vehGo.Config.DisplayName, modelPrice, JsonConvert.SerializeObject(vehGo.Data.VehCustomization), player.Vehicle.Class, cntTurboStage);
            }
        ...
        public static void RemoteEvent_exitTuning(PlayerGo player)
        {
            try
            {
        ...
        public static void RemoteEvent_lsCustomBuyTuning(PlayerGo player, params object[] arguments)
        {
            try
            {
                ...
                int bizID = player.GetCharacter().BusinessInsideId;

                if (type == ModTypes.Livery)
                {
                    if (ExclusiveLivery.ContainsKey(player.Vehicle.Model) && ExclusiveLivery[player.Vehicle.Model].Contains(variant) && bizID != -2 && player.GetCharacter().AdminLVL < 8)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "local_88", 3000);
                        return;
                    }
                }
                CustomizationVehicleModel data = player.Vehicle.GetVehicleGo().Data.VehCustomization;
                ...
                if (wheelsType != -2)
                    VehicleCustomization.SetMod(player.Vehicle, ModTypes.WheelsType, wheelsType);
                VehicleCustomization.SetMod(player.Vehicle, type, variant);
        ...
        public static void RemoteEvent_lsCustomBuyColor(PlayerGo player, params object[] arguments)
        {
            try
            {
                if (!player.IsLogged()) return;
                ...
                int bizID = player.GetCharacter().BusinessInsideId;
                CustomizationVehicleModel data = vehicle.GetVehicleGo().Data.VehCustomization;
        ...
        public static void RemoteEvent_lsCustomBuyNeon(PlayerGo player, params object[] arguments)
        {
            try
            {
                if (!player.IsLogged()) return;
        ...
        private static bool PaidForBuyTuning(Player player, int priceInPart, int bizID)
        {
            ...
            return BusinessManager.TakeProd(player, biz, player.GetCharacter(), 
                new BuyModel("Parts", priceInPart, true, (cnt) => cnt), 
                "Money_BuyTuning".Translate(player.Vehicle.GetVehicleGo().Data.ID), null);
        }

        private static void UpdateTuning(Player player, CustomizationVehicleModel data)
        {
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Biz_128", 3000);
            Trigger.ClientEvent(player, "tuningUpd", JsonConvert.SerializeObject(data));
        }

        public static string GetVehicleModel(Vehicle vehicle)
        {
            string vehModel = vehicle.GetVehicleGo().Data.ModelName;
            if (vehModel == null)
