...
namespace Whistler.Core
{
    ...
    partial class BusinessManager : Script
    {
        ...
        public static void RemoteEvent_cancelTattooRemove(Player player)
        {
            try
            {
        ...
        public static void RemoteEvent_buyTattoo(Player player, int index)
        {
            try
            {
                var customization = player.GetCustomization();
                if(index < 0 || customization.Tattoos.Count > index)
                {
                    ...
                    if(config == null)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "tattoo:shop:remove:err", 3000);
                        return;
                    }

                    if (player.GetCharacter().Money < config.Price)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Biz_1", 3000);
                        return;
                    }
                    Wallet.MoneySub(player.GetCharacter(), config.Price, "Money_RemoveTattoo");
                    customization.Tattoos.RemoveAt(index);
                    customization.Apply(player);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "tattoo:shop:remove:succ", 3000);
                    CustomizationService.UpdateTattoos(customization);
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "tattoo:shop:remove:err", 3000);
        ...
        public static void RemoteEvent_buyTattoo(Player player, int zone, int tattooID)
        {
            try
            {
                ...
                TakeProd(player, biz, player.GetCharacter(), new BuyModel("Tattoos", priceModel.MaterialsAmount, true, 
                    (cnt) =>
                    {
                        var tattooHash = player.GetGender() ? tattoo.MaleHash : tattoo.FemaleHash;

                        var tattoos = player.GetCustomization().Tattoos;
                        tattoos.Add(new Decoration { Collection = tattoo.Dictionary, Overlay = tattooHash });

                        var customization = player.GetCustomization();
                        customization.Apply(player);
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Biz_155".Translate(tattoo.Name, priceModel.Price), 3000);
                        CustomizationService.UpdateTattoos(customization);
                        return cnt;
                    }), "Money_BuyTattoo", null);
