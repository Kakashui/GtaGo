...
namespace Whistler.Businesses
{
    ...
    {
        private static int _countBuy = 10;
        public static void OpenFarmMenu(Player player, Business business)
        {
            player.GetCharacter().BusinessInsideId = business.ID;
            var experience = player.GetWorker().GetExp(FarmManager.Job);
            player.TriggerEvent("farm::openMenu", experience, JsonConvert.SerializeObject(business.Products));
        }

        [RemoteEvent("farmHouse:buyProduct")]
        public static void FarmHouseBuyProduct(Player player, string productName)
        {
            ...
            if (item == null)
            {
                Notify.SendError(player, "Frac_148");
                return;
            }
            BusinessManager.TakeProd(player, business, player.GetCharacter(), 
                new BuyModel(
                    productName, 
                    item.Count, 
                    true, 
