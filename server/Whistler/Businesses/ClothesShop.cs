...
namespace Whistler.Core
{
    partial class BusinessManager : Script
    {
        ...
        public static void RemoteEvent_buyClothes(PlayerGo player, int type, int variation, int texture)
        {
            try
            {
                ...
                if (addItem == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Biz_102", 3000);
                    return;
                }

                var priceModel = biz.GetProductPriceByProductId(0, tempPrice);

                BusinessManager.TakeProd(player, biz, player.GetCharacter(),
