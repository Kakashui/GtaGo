...
namespace Whistler.VehicleTrading
{
    class TradeManager : Script
    {
        ...
        public static void OpenBuyMenu(Player player, int point)
        {
            if (!_tradePoints.ContainsKey(point))
                return;
            var tradePoint = _tradePoints[point];
        ...
        public static void SetVehicleOnTradePoint(Player player, int point, int price)
        {
            if (!player.IsLogged())
                return;
            if (!_tradePoints.ContainsKey(point))
            ...
            if (vehGo.Data.OwnerType != OwnerType.Personal || !vehGo.Data.CanAccessVehicle(player, VehicleSystem.AccessType.SellDollars))
            {
                Notify.SendError(player, "trade:mgr:2");
                return;
            }
            if (!VehicleOperations.CheckCorrectVehiclePrice(player, vehGo.Data.ModelName, price))
                return;
            var tradePoint = _tradePoints[point];
            if (tradePoint.SetVehicle(vehGo.Data as PersonalBaseVehicle, price))
            {
        ...
        public static void BuyVehicle(Player player, int point)
        {
            if (!_tradePoints.ContainsKey(point))
                return;
            var tradePoint = _tradePoints[point];
        ...
        public static void CreateTradePoint(Player player)
        {
            if (!player.IsLogged())
                return;
        ...
        public static void DeleteTradePoint(Player player, int id)
        {
            if (!player.IsLogged())
                return;
