...
namespace Whistler.VehicleTrading.Models
{
    class VehicleTrade
    {
        ...
        private void OpenSellMenu(Player player)
        {
            ...
            if (vehGo.Data.OwnerType != OwnerType.Personal || !vehGo.Data.CanAccessVehicle(player, AccessType.SellDollars))
            {
                Notify.SendError(player, "veh:trader:menu:3");
                return;
            }
            player.TriggerEvent("vehTrade::openSellMenu", Id, vehGo.Config.DisplayName);
        }
        ...
        public void OpenBuyMenu(Player player)
        {
            ...
            player.TriggerEvent("vehTrade::openBuyMenu", Id, Price, vehDataPersonal.Number);
        }

        public bool BuyVehicle(Player player)
        {
            var vehData = VehicleManager.GetVehicleBaseByUUID(CurrVehicle);
            if (vehData == null || !(vehData is PersonalBaseVehicle))
                return false;
            var sellerUUID = vehData.OwnerID;
            if (sellerUUID == player.GetCharacter().UUID)
                return false;
            ...
            if (!Wallet.TransferMoney(player.GetBankAccount(), new List<(IMoneyOwner, int)>
                {
            ...
            vehData.OwnerID = player.GetCharacter().UUID;
            vehData.Save();
            GUI.MainMenu.SendProperty(player);
            var seller = Main.GetPlayerByUUID(sellerUUID);
