...
namespace Whistler.MiniGames.MakeWeapon
{
    public static class MakeWeaponService
    {
        ...
        internal static void GetDetail(Player player)
        {
            if (Job.IsOnJub(player))
            {
                player.TriggerEvent("mw:game:detail:get");
            }
        }

        internal static void SetLimit(Player player, int limit)
        {
        ...
        public static void HandleGameResult(Player player, int id, int result, int bonus)
        {
            var worker = player.GetWorker();
            if (result < 1)
            {
                if (Wallet.MoneySub(player.GetCharacter(), GameCost, "Money_MakeWeaponGame"))
                    player.TriggerEvent("mg:makeweapon:stage:next", worker.Expiriance, 0);
                else
                    QuitJob(player);
            }
            else
            {
                ...
                if (player.GetCharacter().FractionID == 14)
                    bonus += 3;

                if (bonus > 0)
                    total += (int)Math.Floor(total * bonus * .1);
                total -= GameCost;
                total = NewDonateShop.DonateService.UseJobCoef(player, total);
                Wallet.MoneyAdd(player.GetCharacter(), total, "Money_MakeWeaponGame");
                worker.TotalInPayday += total;
                
                //if (Job.IsLimit(player, worker))
                //{
                //    player.TriggerEvent("mg:makeweapon:quit");
                //    Notify.Send(player, NotifyType.Info, NotifyPosition.Center, "qp:mg:mw:17", 3000);
                //}                    
                //else
                player.TriggerEvent("mg:makeweapon:stage:next", worker.AddExp(Job), total);
            }
        }
        ...
        public static void Init()
        {
            ...
            Job = new Job
            {
                ...
                Condition = (Player player) => player.GetCharacter().FractionID == 14 || player.CheckLic(GUI.Documents.Enums.LicenseName.MakingWeapon),
                Limit = 50000
            };
        ...
        private static void GetOnJub(PlayerGo player, QuestPed ped)
        {
