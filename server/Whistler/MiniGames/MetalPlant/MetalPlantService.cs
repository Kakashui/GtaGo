    ...
    public static class MetalPlantService
    {
        ...
        public static void Init()
        {
            Job = new Job
            {
                ...
                Condition = (Player player) => player.CheckLic(GUI.Documents.Enums.LicenseName.MetalPlantWorker),
                Limit = 50000
            };
            Job.ParseLevels("interfaces/gui/src/configs/gameMetalPlant/levels.js");
        }
        public static void WorkBegine(PlayerGo player)
        {
            ...
            player.TriggerEvent("mg:metalplant:game:open", JsonConvert.SerializeObject(allOre), JsonConvert.SerializeObject(allCoal), Job.GetLvlLvl(player));
        }

        internal static void CalculateGameResult(PlayerGo player, int percent)
        {
            ...
            player.TriggerEvent("mg:metalplant:result", diamond, metals);
            GiveResources(player, perfect, common, diamond);
            if (diamond)
                player.CreatePlayerAction(PersonalEvents.PlayerActions.MakeDiamond, 1);
        }
        private static void GiveResources(PlayerGo player, int perfect, int common, bool diamond)
        {
