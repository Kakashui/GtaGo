...
namespace Whistler.Families.FamilyMP.Models.MPModels
{
    class IslandCaptureMP : FamilyMPModel
    {
        ...
        public static Action<PlayerGo, QuestPed> WorkPed_PlayerInteracted { get; private set; }
        ...
        private static void EnterIsIslandPed(Player player)
        {
        ...
        private static void ExitIsIslandPed(Player player)
        {
            player.ChangePosition(_enterPoint + new Vector3(0, 0, 1.5), 2000);
        }

        private static void EnterIsland(Player player)
        {
            if (!player.IsInVehicle || player.VehicleSeat != VehicleConstants.DriverSeat)
            {
                Notify.SendError(player, "qp:isle:3");
                return;
            }
            bool isIslandCapt = (ManagerMP.CurrentMP?.Type ?? FamilyMPType.Invalid) == FamilyMPType.IslandCapture;
            Vector3 targetPosition = isIslandCapt ? _outPoints.GetRandomElement() : _exitPoint;
        ...
        private static void ExitIsland(Player player)
        {
        ...
        public void PlayerEnterZone(Player player, BattleLocation location)
        {
        ...
        public void PlayerExitZone(Player player, BattleLocation location)
        {
            if (location != Location)
                return;
            RemoveMember(player);
        }
        private static bool RemoveMember(Player player)
        {
            if (Players.ContainsKey(player))
            {
        ...
        public override bool TryStartMP()
        {
            ...
            var players = Main.GetPlayersGoListByPredicate((player) => player.Character.IsAlive && player.Character.WarZone == Location);
            ...
            Chat.AdminToAll("isle:start".Translate(NameMP));
        ...
        public override void TryEndMP()
        {
            if (StartDate.AddMinutes(_minuteBeforeEndMP) > DateTime.Now)
                return;
            var familyPlayers = Players.Where(item => item.Value > 0 && (!item.Key.HasSharedData("INVISIBLE") || !item.Key.GetSharedData<bool>("INVISIBLE")));
            ...
            {
                MoneySystem.Wallet.MoneyAdd(player.Key.GetCharacter(), 2000, "Money_FamilyMPIsland");
            }
            ...
            if (WinnerFamily > 0)
                Chat.AdminToAll("isle:end:1".Translate(WinnerFamilyName));
            else
                Chat.AdminToAll("isle:end:2");
        }

        private static void LoadZone(Player player)
        {
            if (!player.IsLogged())
                return;
            ...
            player.TriggerEvent("islandCapt::loadZone", startTime, totalTime, constrictionTime, _startZone.Center, _startZone.Range, _endZone.Center, _endZone.Range);
        }

        private static void UnloadZone(Player player)
        {
            player.TriggerEvent("islandCapt::unloadZone");
        }
        ...
        public override bool PlayerDeath(Player player, Player killer, uint weapon)
        {
            ...
            if (killer != null && killer != player && IsMember(killer))
            {
                ...
                Chat.SendToAdmins(1, "isle:capt:1".Translate(killer.Name, player.Name));
            WhistlerTask.Run(() =>
            {
                player.Health = 0;
            }, 5000);
