...
namespace Whistler.Families
{
    class BusinessWar
    {
        ...
        public event Action<Player, BattleLocation> PlayerEnterBattleLocation;
        public event Action<Player, BattleLocation> PlayerExitBattleLocation;
        ...
        private void onPlayerEnterBizWar(ColShape shape, Player player)
        {
            try
            {
                if (!player.IsLogged())
                    return;
                player.GetCharacter().WarZone = Id;
                PlayerEnterBattleLocation?.Invoke(player, Id);
                if (warIsGoing && (player.GetCharacter().FamilyID == attackersFamily?.Id || player.GetCharacter().FamilyID == defendersFamily?.Id || player.IsAdmin()))
                {
        ...
        private void onPlayerExitBizWar(ColShape shape, Player player)
        {
            try
            {
                if (!player.IsLogged())
                    return;
                player.GetCharacter().WarZone = BattleLocation.None;
        ...
        public bool CMD_startBizwar(Family familyAttack, Family familyDefend, BattleModel battleModel)
        {
            ...
            WarBlip.Color = 49;
            Chat.SendFamilyMessage(familyDefend.Id, "Frac_445".Translate(familyAttack.Name), false);
            Chat.SendFamilyMessage(familyAttack.Id, "Frac_446", false);
            ...
            SendCaptureData((Player p) =>
            {
                CaptureUI.SendUntilCaptureTimer(p, FamilyWars.WarManager.TimeToCapt, 0, "timer_to_capture_1");
            }, true);
        ...
        private void timerStart()
        {
            ...
            Chat.SendFamilyMessage(defendersFamily.Id, "Frac_447".Translate(attackersFamily.Name), false);
            Chat.SendFamilyMessage(attackersFamily.Id, "Frac_448", false);
        }
        ...
        private void EndCapture()
        {
            try
            {
                ...
                {
                    Chat.SendFamilyMessage(defendersFamily.Id, "Frac_449", false);
                    Chat.SendFamilyMessage(attackersFamily.Id, "Frac_450", false);
                }
                else if (attackers > defenders)
                {
                    Chat.SendFamilyMessage(defendersFamily.Id, "Frac_451", false);
                    Chat.SendFamilyMessage(attackersFamily.Id, "Frac_452", false);
                }
                foreach (var member in FamilyManager.GetFamilyMembers(familyWinner.Id))
                {
                    Wallet.MoneyAdd(member.GetCharacter(), _winBizMoney, "Money_WinBiz");
                }
                warIsGoing = false;
                WarBlip.Color = 40;
            }
        ...
        private void SendCaptureData(Action<Player> action, bool timerToCapt = false)
        {
            if (attackersFamily != null)
                foreach (var member in attackersFamily.OnlineMembers)
                {
                    if (member.Value.IsLogged() && (member.Value.GetCharacter().WarZone == Id || timerToCapt))
                    {
            ...
            if (!timerToCapt)
                ...
                {
                    if (admin.IsLogged() && admin.GetCharacter().WarZone == Id)
                    {
                        action.Invoke(admin);
                    }
                }
        ...
        public void UpdateCaptureUi(Player newPlayer = null, bool enable = true)
        {
            var attackers = attackersFamily?.GetMembersInBattle(Id) ?? 0;
            var defenders = defendersFamily?.GetMembersInBattle(Id) ?? 0;
        ...
        public bool Event_PlayerDeath(PlayerGo player, PlayerGo killer, uint weapon)
        {
            if (!warIsGoing) 
                return false;
            if (!player.IsLogged())
