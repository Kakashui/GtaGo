    ...
    class Company
    {
        ...
        private void CreateInteract()
        {
            ...
            Shape = InteractShape.Create(Position, 2, 2)
                .AddInteraction(ActionShape, "interact_45");
        }
        ...
        private void ActionShape(Player player)
        {
            ...
            {
                if (Fractions.Manager.IsGang(player) || player.GetCharacter().FractionID == 16)
                {
                    ownerId = player.GetCharacter().FractionID;
            ...
            List<Player> _rangePlayers = family.OnlineMembers.Values.Where(t => player.Position.DistanceTo(t.Position) < 50f).ToList();
            if (_rangePlayers == null || _rangePlayers.Count < 3) { Notify.SendError(player, "Недостаточно людей"); return; } //от 3 чел 
        ...
        public bool DisconnectedPlayer(Player player)
        {
            if (_attaker == player)
            {
        ...
        public bool PlayerDeath(Player player)
        {
            if (_attaker == player)
            {
                _attaker.TriggerCefEvent("hud/warForEnterprice/setWarForEnterprice", JsonConvert.SerializeObject(new
