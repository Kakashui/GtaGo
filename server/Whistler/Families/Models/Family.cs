    ...
    internal class Family : IBankAccount, IOrganization
    {
        ...
        private void UpdateClothesPoint()
        {
            ...
            _clothesShape = InteractShape.Create(ClothesPoint, 1, 2, ClothesDimension)
                .AddInteraction(SkinManager.OpenFamilySkinMenu, "interact_3")
        ...
        private bool EnterPredicate(ColShape shape, Player player)
        {
            if (!player.IsLogged())
                return false;
        ...
        {
            return IsOwner(player?.GetCharacter()?.UUID ?? -1);
        }
        ...
        {
            return IsLeader(player?.GetCharacter()?.UUID ?? -1);
        }
        ...
        public bool CanAccessToHouse(Player player, FamilyHouseAccess houseAccess)
        {
            if (!CheckPlayer(player))
                return false;
            return IsLeader(player) || Ranks[Members[player.GetCharacter().UUID].Rank].AccessHouse >= houseAccess;
        }

        public bool CanAccessToFurniture(Player player, FamilyFurnitureAccess furnAccess)
        {
            if (!CheckPlayer(player))
                return false;
            return IsLeader(player) || Ranks[Members[player.GetCharacter().UUID].Rank].AccessFurniture >= furnAccess;
        }

        public bool CanAccessToVehicle(Player player, int vehicleID, FamilyVehicleAccess vehicleAccess)
        {
            if (!CheckPlayer(player))
                return false;
            if (Owner == player.GetCharacter().UUID)
                return true;
            return IsLeader(player) || Ranks[Members[player.GetCharacter().UUID].Rank].AccessVehicles.GetValueOrDefault(vehicleID, FamilyVehicleAccess.None) >= vehicleAccess;
        }

        public bool CanAccessToClothes(Player player)
        {
            if (!CheckPlayer(player))
                return false;
            return IsLeader(player) || Ranks[Members[player.GetCharacter().UUID].Rank].AccessClothes;
        }

        public bool CanAccessToBizWar(Player player)
        {
            if (!CheckPlayer(player))
                return false;
            return IsLeader(player) || Ranks[Members[player.GetCharacter().UUID].Rank].AccessBizWar;
        }
        public bool CanAccessToMemberManagement(Player player)
        {
            if (!CheckPlayer(player))
                return false;
            return IsLeader(player) || Ranks[Members[player.GetCharacter().UUID].Rank].AccessMemberManagement;
        }
        #endregion
        ...
        public bool DeleteMember(int uuid)
        {
            ...
            if (player != null)
            {
                if (player.GetCharacter().FamilyID == Id)
                {
                    player.GetCharacter().FamilyID = 0;
                    player.GetCharacter().FamilyLVL = 0;
                }
                ...
                player.SetSharedData("familyname", "None");
            }
        ...
        {
            SetOwner(player.GetCharacter().UUID);
        }
        ...
        {
            return OnlineMembers.Where(item => item.Value.IsLogged() && item.Value.GetCharacter().IsAlive && item.Value.GetCharacter().WarZone == id).Count();
        }
