...
namespace Whistler.AntiCheatServices
{
    internal class AntiCheatService : Script
    {
        ...
        [RemoteEvent("ac:w:fb")]
        public void OnSpeedLimitExceeded(PlayerGo player, uint weapon)
        {
            if (!player.IsLogged() || player.GetCharacter().AdminLVL > 0) return;
            Chat.SendToAdmins(1, $"[AntiCheat]: [{player.GetCharacter().UUID}]{player.Name} banned: forbidden weapon");
            _logger.WriteWarning($"forbidden weapon (${player.Name}) : {weapon}");
            Admin.hardbanPlayer(player, 9999, "cheat w1");
        }

        [RemoteEvent("ac:teleport")]
        public static void OnTeleportCheat(PlayerGo player, string positionPrev, string positionNew, int speed, int dist)
        {
            if (player.IsAdmin())
                return;
            if (player.HasData("lastTeleport"))
            {
                ...
                if(dateTime < DateTime.Now)
                    Chat.SendToAdmins(2, $"[AntiCheat]: [{player.GetCharacter().UUID}]{player.Name} (teleport). TP from {positionPrev} to {positionNew}. Speed {speed}, Distance: {dist}");
            }
            else
                Chat.SendToAdmins(2, $"[AntiCheat]: [{player.GetCharacter().UUID}]{player.Name} (teleport). TP from {positionPrev} to {positionNew}. Speed {speed}, Distance: {dist}");
        }

        public static void ArmorHack_BadArmor(PlayerGo player, int armor) {
            _logger.WriteWarning($"ArmorHack:\n{player.Name} BodyArmour:{armor} / player:{player.Armor}");
        }

        public static void ArmorHack_NoArmor(PlayerGo player, int clientArmour)
        {
            _logger.WriteWarning($"ArmorHack_NoArmor:\n{player.Name} not have BodyArmour but  client armour = {clientArmour}");
        }

        public static void BadClientWeaponHandle(PlayerGo player, string msg)
        {
            _logger.WriteWarning($"bad client weapon (${player.Name}): {msg}");
            var message = "ach:bcwep".Translate(player.GetCharacter().UUID);
            SendToAdminChat(message);
        }

        public static void KillPlayerHandle(PlayerGo player, PlayerGo killer, uint weapon)
        {
            if (killer.IsLogged())
            {
        ...
        private static void SendToAdminChat(string msg)
        {
            ...
            {
                if (p.Character.AdminLVL >= 2)
                    p.TriggerEvent("chat:api:action", ChatType.AdminChat, msg, -1);
            });
        }

        [RemoteEvent("__ragemp_cheat_detected")]
        public void CheatDetect1(PlayerGo player, string cheatCode)
        {
            _logger.WriteWarning($"ragemp_cheat_detected1: {cheatCode}");
        }

        [RemoteEvent("_ragemp_cheat_detected")]
        public void CheatDetect2(PlayerGo player, string cheatCode)
        {
            _logger.WriteWarning($"ragemp_cheat_detected2: {cheatCode}");
        }

        [RemoteEvent("ragemp_cheat_detected")]
        public void CheatDetect3(PlayerGo player, string cheatCode)
        {
