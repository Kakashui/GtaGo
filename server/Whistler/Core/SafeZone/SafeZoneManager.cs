...
namespace Whistler.Core.SafeZone
{
    class SafeZoneManager : Script
    {
        ...
        private static void PlayerReady(Player player)
        {
            var offZones =  _safezones.Where(item => !item.Value.Active).Select(item => item.Key).ToList();
            Trigger.ClientEvent(player, "safeZones:setInActiveZones", offZones);
        }

        [Command("safezonechange")]
        public void CMD_safezonechange(Player player, int id)
        {
            if (!player.IsLogged())
                return;
            ...
            if (_safezones[id].Active)
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "sfzone:on".Translate(id), 3000);
            else
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "sfzone:off".Translate(id), 3000);
