...
namespace Whistler.Core.SafeZone
{
    public class SafeZoneModel
    {
        ...
        private void OnEntityEnterColShape(PlayerGo player)
        {
            try
            {
                if (!player.IsLogged())
                ...
                player.TriggerEvent("safeZone", Id, true);
            }
            catch (Exception e) { _logger.WriteError($"SafeZoneEnter: {e.ToString()}"); }

        }
        private void OnEntityExitColShape(PlayerGo player)
        {
            try
            {
                if (!player.IsLogged())
                ...
                player.TriggerEvent("safeZone", Id, false);
            }
        ...
        public void SetActive(bool active)
        {
            if (Active == active)
                return;
            Active = active;
            Trigger.ClientEventToPlayers(NAPI.Pools.GetAllPlayers().ToArray(), "safeZones:setActiveZone", Id, Active);
        }
    }
}
