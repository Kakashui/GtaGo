...
namespace Whistler.Core
{
    class Weapons : Script
    {
        ...
        public static void Event_PlayerDeath(PlayerGo player, PlayerGo killer, uint reason)
        {
            try
            {
        ...
        public static void OnUseItemBox(PlayerGo player, ItemBox box)
        {
            ...
            {
                box.CountStoredItem--;
                GameLog.Box(box.Serial, player.GetCharacter().UUID, additem.Name.ToString());
            }
        }

        public static void PickupItemBox(PlayerGo player, int itemId, ItemTypes itemType)
        {
        ...
        public static void OpenFriskMenu(Player player, Player target)
        {
            ...
            player.SetData("frisktarget", target);
            player.TriggerEvent("friskInterface:openmenu", JsonConvert.SerializeObject(list), target.Name, target.GetCharacter().UUID);
        }
        //private const int MusketShotsStrength = 200;
        //[RemoteEvent("weapons:musketShot")]
        //public static void HandleMusketShot(Player player)
        //{
        //    try
        //    {
        //        if (!player.IsLogged()) return;
