...
namespace Whistler.Core.BonusCodes
{
    class BonusCodeManager : Script
    {
        ...
        private static void CreateBonusCode(Player player, string name, int money, int coins, int prime, int length)
        {
            try
            {
        ...
        private static void DeleteBonusCode(Player player, string name)
        {
        ...
        [Command("bonuscode")]
        private static void UseBonusCode(PlayerGo player, string name)
        {
            ...
            if (bonus.Coins > 0)
            {
                player.AddGoCoins(bonus.Coins);
                NewDonateShop.DonateLog.OperationLog(player, bonus.Coins, $"bonuscode({bonus.Name})");
            }
            if (bonus.Money > 0)
            {
                Wallet.MoneyAdd(player.GetCharacter(), bonus.Money, "Money_Bonuscode".Translate(bonus.Name));
            }
