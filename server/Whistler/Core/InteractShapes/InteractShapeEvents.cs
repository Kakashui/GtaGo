...
namespace Whistler.Core.InteractShapes
{
    ...
    {
        public static event Action<PlayerGo> PlayerConnected;

        private static WhistlerLogger _logger = new WhistlerLogger(typeof(InteractShapeEvents));

        [ServerEvent(Event.PlayerConnected)]
        public void HandlePlayerConnect(PlayerGo player)
        {
            try
            {
        ...
        public void InteractKeyPressed(PlayerGo player, int keyNum)
        {
            try
            {
                var key = (Key)keyNum;
