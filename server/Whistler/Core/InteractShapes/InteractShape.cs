...
namespace Whistler.Core
{
    internal class InteractShape
    {
        ...
        private static void LoadMarkersForPlayer(Player player)
        {
        ...
        public static void PlayerPressInteractKeyHandlerStatic(PlayerGo player, Key key)
        {
        ...
        private Action<ColShape, PlayerGo> _enterColshapeHandler;
        private Action<ColShape, PlayerGo> _exitColshapeHandler;
        private Func<ColShape, PlayerGo, bool> _enterPredicate;
        ...
        private InteractShape(int id, Vector3 position, float range, int height, uint dimension = 0)
        {
            ...
            _colshape.OnEntityEnterColShape += (c, p) => EnterInteractColshapeHandler(c, p as PlayerGo);
            _colshape.OnEntityExitColShape += (c, p) => ExitInteractColshapeHandler(c, p as PlayerGo);

        }
        private void PlayerPressInteractKeyHandler(PlayerGo player, Key key)
        {
            try
            {
        ...
        public InteractShape AddEnterPredicate(Func<ColShape, PlayerGo, bool> enterPredicate)
        {
        ...
        public InteractShape DeleteMarker()
        {
            if (_markerInfo != null)
            {
                Trigger.ClientEventToPlayers(NAPI.Pools.GetAllPlayers().ToArray(), "interact:destroyMarker", _id);
                _markerInfo = null;
            }
            return this;
        }

        public InteractShape AddInteraction(Action<PlayerGo> action, string text = "interact_0", Key key = Key.VK_E)
        {
        ...
        public InteractShape AddOnExitColshapeExtraAction(Action<ColShape, PlayerGo> action)
        {
        ...
        public InteractShape AddOnEnterColshapeExtraAction(Action<ColShape, PlayerGo> action)
        {
        ...
        private void CreateMarker(MarkerInfo markerInfo)
        {
            ...
            Trigger.ClientEventToPlayers(NAPI.Pools.GetAllPlayers().ToArray(), "interact:loadMarkers", JsonConvert.SerializeObject(dto));
        }

        private void EnterInteractColshapeHandler(ColShape colShape, PlayerGo client)
        {
            try
            {
                if (!client.IsLogged())
                ...
                client.TriggerEvent("interact:enterInteractShape", JsonConvert.SerializeObject(_interactions.Values.Select(i => i.Dto).ToList()));
        ...
        private void ExitInteractColshapeHandler(ColShape colShape, PlayerGo client)
        {
            try
            {
                if (!client.IsLogged())
                ...
                if(client.DATA_INTERACT_ID == _id)
                {
                    ...
                    client.TriggerEvent("interact:exitInteractShape");
                }
        ...
        private class Interaction
        {
            ...
            public Interaction(Action<PlayerGo> action, Key key, string text)
            {
                Key = key;
                Text = text;
                Action = action;
