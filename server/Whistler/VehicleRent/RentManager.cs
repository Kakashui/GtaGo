...
namespace Whistler.VehicleRent
{
    class RentManager : Script
    {
        ...
        {
            player.TriggerEvent("vehicleRent:loadPeds", JsonConvert.SerializeObject(_rentPoints.Select(item => new { id = item.Value.ID, position = item.Value.PositionPed, heading = item.Value.RotationPed })));
        }

        [RemoteEvent("vehicleRent:tryRent")]
        public static void TryRentVehicle(Player player, int pointId, string model, int category, int payment)
        {
            if (!player.IsLogged())
                return;
        ...
        [Command("setrentpos")]
        public static void SetRentVehiclePosition(Player player)
        {
            if (!player.IsLogged())
                return;
            ...
            player.SetData("VEHICLERENT::POSITION", player.Vehicle.Position);
            player.SetData("VEHICLERENT::ROTATION", player.Vehicle.Rotation.Z);
            Core.Chat.SendTo(player, "vehicleRent_16");
        }
        [Command("createrentpoint")]
        public static void CreateRentPoint(Player player)
        {
        ...
        public static void ChangeRentPedPosition(Player player, int pointId)
        {
            if (!player.IsLogged())
                return;
            if (!_rentPoints.ContainsKey(pointId))
        ...
        public static void ChangeRentCarPosition(Player player, int pointId)
        {
            if (!player.IsLogged())
                return;
            if (!player.IsInVehicle)
            ...
            if (!_rentPoints.ContainsKey(pointId))
                return;
            _rentPoints[pointId].ChangePositionCar(player.Vehicle);
            Core.Chat.SendTo(player, "vehicleRent_16");
        }
        [Command("addrentcategory")]
        public static void AddRentVehicleCategory(Player player, int pointId, int category)
        {
            if (!player.IsLogged())
                return;
            if (!_rentPoints.ContainsKey(pointId))
        ...
        public static void DeleteRentVehicleCategory(Player player, int pointId, int category)
        {
            if (!player.IsLogged())
                return;
            if (!_rentPoints.ContainsKey(pointId))
        ...
        [Command("deleterent")]
        public static void DeleteRentVehicle(Player player, int pointId)
        {
            if (!player.IsLogged())
                return;
            if (!_rentPoints.ContainsKey(pointId))
