    ...
    class RentPoint
    {
        ...
        private void CreateInteract()
        {
            ...
            Shape = InteractShape.Create(PositionPed, 2, 2)
                .AddInteraction(ActionShape, "interact_44".Translate(ID));
        }

        private void ActionShape(Player player)
        {
            if (!player.IsLogged())
                return;
            player.TriggerEvent("vehicleRent:openMenu", ID, JsonConvert.SerializeObject(Categories));
        }

        public void TryRentVehicle(Player player, string model, int category, int payment)
        {
        ...
        public void CreateRentVehicle(Player player, CategoryModel categoryModel, RentModel model, int payment)
        {
            if (!MoneySystem.Wallet.MoneySub(player.GetMoneyPayment((MoneySystem.PaymentsType)payment), model.Price, "Money_Rent".Translate(model.Model)))
            {
        ...
        public void ChangePositionPed(Player player)
        {
        ...
        public void ChangePositionCar(Vehicle vehicle)
        {
