...
namespace Whistler.DoorsSystem
{
    public static class DoorsService
    {
        ...
        public static void ParseDoorsConfigs()
        {
            ...
            {
                using (var w = new StreamWriter("Client/src/client/doors/configs.js"))
                {
        ...
        public static void SetDoorState(Player player, int hash, bool state)
        {
            ...
            door.SetState(state);
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, door.Locked ? "door_1" : "door_0", 3000);
        }

        public static void SyncDoorStateForPlayer(Player player)
        {
            player.TriggerEvent("doors:state:sync", _config.Changed);
        }

        private static bool HasDoorAccess(Player player, DoorComplect door)
        {
            if (player.GetCharacter().AdminLVL > 2) return true;
            if (player.GetCharacter().MediaHelper > 0) return true;
            if (!_accessList.ContainsKey(door.Hash)) return false;
            var access = _accessList[door.Hash];
        ...
        public static void AddDoorAccess(Player player,int uuid, string access)
        {
            if (_accessList.ContainsKey(uuid))
            {
                _accessList[uuid] = access;
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "door:acc:change".Translate(access), 3000);
                MySQL.Query("UPDATE `dooraccess` SET `accessname`=@prop0 WHERE `uuid`=@prop1", access, uuid);
            }
            else
            {
                _accessList.Add(uuid, access);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "door:acc:add".Translate(access), 3000);
                MySQL.Query("INSERT INTO `dooraccess` (`uuid`,`accessname`) VALUES (@prop0, @prop1)", uuid, access);
            }
        }

        public static void CheckDoorAccess(Player player, int uuid)
        {
            if (_accessList.ContainsKey(uuid))
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "door:acc:check".Translate(_accessList[uuid]), 3000);
            else
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "door:acc:no", 3000);
        }
    }
}
