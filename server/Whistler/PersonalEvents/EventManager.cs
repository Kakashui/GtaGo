...
namespace Whistler.PersonalEvents
{
    ...
    {
        private static readonly Dictionary<PlayerActions, Action<PlayerGo, int>> _subscribes = new Dictionary<PlayerActions, Action<PlayerGo, int>>();
        public EventManager()
        {
        ...
        private void FamilyLoad(PlayerGo player, Family family)
        {
            player.TriggerEvent("personalEvents:loadFamilyContracts", family.FamilyContracts.GetSerializeDTOString());
        }

        private void OnPlayerReady(PlayerGo player)
        {
            player.TriggerEvent("personalEvents:load", player.Character.EventModel.GetSerializeDTOString());
            player.TriggerEvent("personalEvents:loadMyContracts", player.Character.EventModel.Contracts.GetSerializeDTOString());
        }

        public static void AddEvent(PlayerActions action, Action<PlayerGo, int> handler)
        {
        ...
        public static void RemoveEvent(PlayerActions action, Action<PlayerGo, int> handler)
        {
            if (_subscribes.TryGetValue(action, out Action<PlayerGo, int> d))
            {
                if (d != null)
                    d -= handler;
            }
        ...
        public static void InvokeEvent(PlayerActions action, PlayerGo player, int progress)
        {
            _subscribes.TryGetValue(action, out Action<PlayerGo, int> d);
            d?.Invoke(player, progress);
        }

        [Command("testevent")]
        public static void CreatePlayerAction(PlayerGo player, int eventName, int progress)
        {
            player.CreatePlayerAction((PlayerActions)eventName, progress);
        }

        [RemoteEvent("personalEvents:syncClientEvents")]
        public static void SyncClientEvents(PlayerGo player, string eventName, int points)
        {
        ...
        public static void AcceptReward(PlayerGo player, int eventName)
        {
        ...
        public static void AcceptContract(PlayerGo player, int contractName)
        {
