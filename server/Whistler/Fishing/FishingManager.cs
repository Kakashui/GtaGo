...
namespace Whistler.Fishing
{
    class FishingManager
    {
        ...
        internal void DoAction(Player client)
        {
            try
            {
                FishingActions action = client.GetData<FishingActions>(Const.DATA_ACTION_ID);
                switch (action)
                {
                    case FishingActions.FishSpot:
                        int id = client.GetData<int>(Const.DATA_SPOT_ID);
                        if (id < 0)
                        {
                            client.TriggerEvent("notify", 4, 9, "Fish_1".Translate(), 3000);
                            return;
                        }
                        if (client.IsInVehicle)
                        {
                            Notify.Send(client, NotifyType.Warning, NotifyPosition.BottomCenter, "Fish_2".Translate(), 3000);
                            return;
                        }
                        var inventory = client.GetInventory();
                        var rod = inventory.Items.GetActiveRod();
                        if (rod == null)
                        {
                            Notify.Send(client, NotifyType.Warning, NotifyPosition.BottomCenter, "Fish_3".Translate(), 3000);
                            return;
                        }

                        var bait = inventory.Items.GetBait();
                        if (bait == null)
                        {
                            Notify.Send(client, NotifyType.Warning, NotifyPosition.BottomCenter, "Fish_4".Translate(), 3000);
                            return;
                        }
                        ...
                        if (cage == null)
                        {
                            Notify.Send(client, NotifyType.Warning, NotifyPosition.BottomCenter, "Fish_5".Translate(), 3000);
                            return;
                        };

                        FisherData fisher = client.GetData<FisherData>(Const.DATA_FISHER);
                        if (fisher.MiniGame.Active)
                        {
                            fisher.MiniGame.Stop();
                            Notify.Send(client, NotifyType.Info, NotifyPosition.BottomCenter, "Fish_7".Translate(), 3000);
                        }
                        else
                        {
                            inventory.SubItemByName(ItemNames.FishingBait, 1, LogAction.Use);
                            Notify.Send(client, NotifyType.Info, NotifyPosition.BottomCenter, "Fish_8".Translate(), 3000);
        ...
        internal void DropFish(Player client, int id, int count)
        {
            try
            {
                var inventory = client.GetInventory();
                ...
                if (cage == null)
                {
                    Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "Fish_9".Translate(), 3000);
                    return;
                }
        ...
        internal void EndMiniGame(PlayerGo client, bool result)
        {
            try
            {
                var inventory = client.GetInventory();
                ...
                if (rod == null)
                {
                    Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "Fish_10".Translate(), 3000);
                    return;
                }
                if (result)
                {
                    var cage = inventory.Items.GetFreeCage();
                    if (cage == null)
                        Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "Fish_5".Translate(), 3000);
                    else
                    {
                        if (rod.CountUsing < 100 && FishingAPI.Random.Next(0, 100) < _rodChance)
                            rod.CountUsing--;
                        ...
                        inventory.UpdateItemData(cage.Index);
                        if (fisher.AddExp())
                            Notify.Send(client, NotifyType.Info, NotifyPosition.BottomCenter, "Fish_11".Translate( FishingAPI.FishShops.GetFishName(fisher.MiniGame.Fish).Translate(), fisher.Lvl), 3000);
                        else
                            Notify.Send(client, NotifyType.Info, NotifyPosition.BottomCenter, "Fish_12".Translate( FishingAPI.FishShops.GetFishName(fisher.MiniGame.Fish).Translate()), 3000);
                        client.CreatePlayerAction(PersonalEvents.PlayerActions.CathAFish, 1);
                    }
                }
                else
                {
                    if (rod.CountUsing < 100)
                        rod.CountUsing--;
                    Notify.Send(client, NotifyType.Info, NotifyPosition.BottomCenter, "Fish_13".Translate(), 3000);
                }

                if (rod.CountUsing < 1)
                {
                    inventory.SubItem(rod.Index);
                    Notify.Send(client, NotifyType.Info, NotifyPosition.BottomCenter, "Fish_14".Translate(), 3000);
                }
            }
        ...
        public void DeleteFishingSpot(Player client)
        {
            try
            {
                var id = client.GetData<int>(Const.DATA_SPOT_ID);
                if (id < 0) return;
        ...
        public void AddFishingSpot(Player client)
        {
            try
            {
        ...
        public void Colsh_OnEntityExitColShape(GTANetworkAPI.ColShape colShape, Player client)
        {
            try
            {
                client.SetData(Const.DATA_SPOT_ID, -1);
                client.SetData(Const.DATA_ACTION_ID, FishingActions.NoAction);
                client.TriggerEvent(Const.CLIENT_EVENT_SHOW_ACTION, false);
            }
        ...
        public void Colsh_OnEntityEnterColShape(GTANetworkAPI.ColShape colShape, Player client)
        {
            try
            {
                client.SetData(Const.DATA_SPOT_ID, colShape.Value);
                client.SetData(Const.DATA_ACTION_ID, FishingActions.FishSpot);
                client.TriggerEvent(Const.CLIENT_EVENT_SHOW_ACTION, true);
            }
