...
namespace Whistler.Fishing.Models
{
    class FisherData
    {
        public FisherData(Player client)
        {
            try
            {
                var query = $"SELECT * FROM `fisher_data` WHERE `socialname`='{client.SocialClubName}';";
                ...
                if (resp.Rows.Count == 0)
                {
                    ...
                    try
                    {
                        query = $"INSERT INTO `fisher_data` (`lvl`, `exp`, `map_expires`, `license`, `socialname`) VALUES({Lvl}, {Exp}, '{MySQL.ConvertTime(DateTime.Now)}', 0, '{client.SocialClubName}'); SELECT LAST_INSERT_ID();";
                        var sub_resp = MySQL.QueryRead(query);
