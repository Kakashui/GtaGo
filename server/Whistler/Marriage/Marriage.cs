...
namespace Whistler.Marriage
{
    public class Marriage : Script
    {
        ...
        public static void Marriage_callbackInvite(PlayerGo player, string name)
        {
            try
            {
                PlayerGo target = Main.GetPlayerByName(name);
                if (target == null)
                {
                ...
                if (target.GetCharacter().Partner != -1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "local_66", 3000);
                    return;
                }
                target.SetData("Marriage:partner", player);
                Trigger.ClientEvent(target, "marriage:proposal", player.Name.Replace('_',' '));
            }
        ...
        public static void Marriage_callbackProposal(PlayerGo target, bool assent)
        {
            try
            {
                ...
                PlayerGo player = target.GetData<PlayerGo>("Marriage:partner");
                if (!player.IsLogged())
                {
                ...
                if (!Wallet.TransferMoney(player.GetCharacter(), Manager.GetFraction(6), _marriageDuty, 0, "Money_Marriage"))
                {
        ...
        public static void InteractMerriage(Player player)
        {
            try
            {
                if (!player.GetGender())
        ...
        public static void Divorce(Player player)
        {
            try
            {
                int partner = player.GetCharacter().Partner;
                if (partner == -1)
                    return;
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "local_75".Translate( Main.PlayerNames[partner]), 3000);
                player.GetCharacter().Partner = -1; 
                Player target = Main.GetPlayerByUUID(partner);
                if (target != null)
                {
                    target.GetCharacter().Partner = -1;
                ...
                MySQL.Query("UPDATE characters SET partner = @prop1 WHERE uuid = @prop0", player.GetCharacter().UUID, -1);          
            }
            catch (Exception ex) { _logger.WriteError("interactPressed: " + ex.ToString()); }
        }
