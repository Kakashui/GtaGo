...
namespace ServerGo.Casino.Games.Poker
{
    internal class PokerGame : BaseCasinoGame
    {
        ...
        /// </summary>
        public void OnPlayerEnterGame(Player player, Gambler gambler)//todo: withdraw this chips
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "poker:enterPoker", Id, _gameMembers[player].FullBank, JsonConvert.SerializeObject(_gameMembers.Values), _gameMembers[player]._place, SmallBlind);
                foreach (var play in _gameMembers.Keys)
                {
                    if (play != null && play != player) //себе не отправляем
                        Trigger.ClientEvent(play, "poker:newPlayer", JsonConvert.SerializeObject(_gameMembers[player]));
                }
                Trigger.ClientEvent(player, "poker:TableCard", JsonConvert.SerializeObject(_dealing._tableCards));
        ...
        /// </summary>
        public void OnPlayerExitGame(Player player, Gambler gambler)
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "roulette:updatePlayerBank", bank.TotalValue, JsonConvert.SerializeObject(CashBoxDto.CreateDto(bank.Chips)));
        ...
        public void OnPlayerPlacedBets(Player player, string bet, int amount)
        {
            try
            {
        ...
        public void OnPlayerBuyChips(Player player, int amount)
        {
            try
            {
                if (!_gameMembers.ContainsKey(player)) return;
