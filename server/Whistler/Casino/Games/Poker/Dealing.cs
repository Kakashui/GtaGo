    ...
    internal class Dealing
    {
        ...
        private void DoBlindsStep()
        {
            try
            {
                ...
                _queue.Peek()._move = "Cas_14".Translate();
                SendMoveOnPlayers(_queue.Peek());
                _queue.Enqueue(_queue.Dequeue());
                _queue.Peek()._move = "Cas_15".Translate();
                    ...
                    switch (bet)
                    {
                        case BetType.Call:
                            ...
                            if (player.Balance >= sum)
                            {
                                ...
                                player._move = "Cas_16".Translate();
                            }
                            else
                            {
                                _dealingRound.SetBetPlayer(player, player.Balance);
                                player._move = "Cas_17".Translate();
                            }
                            break;
                        case BetType.Fold:
                            player.LeaveDealing();
                            player._move = "Cas_18".Translate();
                            break;
                        case BetType.Check:
                            if (_dealingRound.GiveBetPlayer(player) >= _currentBet)
                            {
                                player._move = "Cas_19".Translate();
                            }
                            else
                            {
                                player.LeaveDealing();
                                player._move = "Cas_20".Translate();
                            }
                            break;
                        case BetType.Raise:
                            ...
                            if (player.Balance > amount)
                            {
                                ...
                                player._move = "Cas_21".Translate();
                            }
                            else if (player.Balance == amount)
                            {
                                _dealingRound.SetBetPlayer(player, amount);
                                player._move = "Cas_22".Translate();
                            }
                            else if (player.Balance < amount)
                            {
                                _dealingRound.SetBetPlayer(player, player.Balance);
        ...
        private void WaitNextRound()
        {
            try
            {
                ...
                foreach (var player in _players)
                {
                    Trigger.ClientEvent(player._client, "poker:NewDealing");
                }
                _timer = Timers.Start(1000 * time_wait_newRound, TimerWaitNextRound);
            }
            catch (Exception e)
        ...
        private void SendHandCardToPlayers()
        {
            try
            {
                foreach (var player in _players)
                {
                    Trigger.ClientEvent(player._client, "poker:playersCard", JsonConvert.SerializeObject(_queue), JsonConvert.SerializeObject(player.Cards));
                    if (player._client.HasData("ALLSEEINEYE") && player._client.GetData<bool>("ALLSEEINEYE"))
                        foreach (var play in _players)
                            Trigger.ClientEvent(player._client, "poker:giveMyCards", play._place, JsonConvert.SerializeObject(play.Cards)); //named giveMyCards for conspiration
                }
            }
            catch (Exception e)
            {
        ...
        public void SendEventToMembers(string eventName, params object[] args)
        {
            ...
            {
                foreach (var play in _players)
                    Trigger.ClientEvent(play._client, eventName, args);
            }
            catch (Exception e)
            {
