...
namespace ServerGo.Casino.Business
{
    internal class CashBox
    {
        ...
        public void Withdraw(int dollars, Player player = null)
        {
            Amount -= dollars;
            UpdateDbAmount();
            if (player != null)
                GameLog.Casino(nameof(Withdraw), dollars, player.GetCharacter().FullName, DateTime.Now);
        }

        public void Charge(int dollars, Player player = null)
        {
            Amount += dollars - (long) (dollars * CasinoManager.StateShare);
            UpdateDbAmount();
            if (player != null)
                GameLog.Casino(nameof(Charge), dollars, player.GetCharacter().FullName, DateTime.Now);
        }
