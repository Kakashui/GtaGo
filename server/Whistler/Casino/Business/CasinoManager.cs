...
namespace ServerGo.Casino.Business
{
    ...
    internal class CasinoManager : Script
    {
        ...
        public static Casino FindFirstCasino() => Casinos.Count > 0 ? Casinos[0] : null;
        ...
        public void OnResourceStart()
        {
            ...
            {
                if (BusinessManager.GetBusiness(FindFirstCasino().BizId).OwnerID == player.GetCharacter().UUID)
                {
                    var page1 =
                        new DialogPage("qp:casino:1", ped.Name,
                                ped.Role)
                            .AddCloseAnswer();
                    var page2 =
                        new DialogPage("qp:casino:2", ped.Name,
                                ped.Role)
                            .AddCloseAnswer();
                    var introPage =
                        new DialogPage("qp:casino:3", ped.Name, ped.Role)
                            .AddAnswer("qp:casino:4", page1)
                            .AddAnswer("qp:casino:5", CasinoMenu)
                            .AddAnswer("qp:casino:6", page2)
                            .AddCloseAnswer("qp:casino:7");
                    introPage.OpenForPlayer(player);
                }
                else
                {
                    var introPage =
                        new DialogPage("qp:casino:8", ped.Name, ped.Role)
                            .AddCloseAnswer();
                    introPage.OpenForPlayer(player);
                }
            };
            ...
            InteractShape.Create(EnterToCasinoRoof, 1, 2)
                .AddDefaultMarker()
                .AddInteraction(RoofExitCasino, "interact_28");

            InteractShape.Create(ExitFromCasinoRoof, 1, 2)
                .AddDefaultMarker()
                .AddInteraction(RoofEnterCasino, "interact_28");


        }

        private static void RoofExitCasino(Player player)
        {
            player.ChangePosition(ExitFromCasinoRoof + new Vector3(0, 0, .7));
        }

        private static void RoofEnterCasino(Player player)
        {
            player.ChangePosition(EnterToCasinoRoof + new Vector3(0, 0, .7));
        }

        [Command("casino")]
        public static void CasinoMenu(Player player)
        {
            try
            {
                if (BusinessManager.GetBusiness(FindFirstCasino().BizId).OwnerID != player.GetCharacter().UUID) return;
                player.TriggerEvent(
        ...
        public static void Deposit(Player player, int charge)
        {
            try
            {
                if (charge <= 0 || player.GetCharacter().Money < charge)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Main_93", 3000);
                    return;
                }
                FindFirstCasino().CashBox.Charge(charge);
                player.TriggerEvent("casinoOwner:update", FindFirstCasino().CashBox.Amount);
                Wallet.MoneySub(player.GetCharacter(), charge, "Money_ChargeCasino");
            }
            catch (Exception e)
            {
        ...
        public static void Withdraw(Player player, int withdraw)
        {
            try
            {
                ...
                player.TriggerEvent("casinoOwner:update", FindFirstCasino().CashBox.Amount);
                Wallet.MoneyAdd(player.GetCharacter(), withdraw, "Money_WithdrawCasino");
            }
            catch (Exception e) 
            {
        ...
        public static void BoughtChipsCallback(Player player, int black, int red, int blue, int green, int yellow)
        {
            try
            {
        ...
        public static void SoldChipsCallback(Player player)
        {
            try
            {
        ...
        public static void LeftRouletteCallback(Player player)
        {
            try
            {
        ...
        public static void LeftPokerCallback(Player player)
        {
            try
            {
                FindFirstCasino()
        ...
        public static void SentTimerInfo(Player player, int seconds)
        {
        ...
        public static void PlacedBetCallback(Player player, string bet, int black, int red, int blue, int green, int yellow)
        {
            try
            {
                var dto = new CashBoxDto
        ...
        public static void PokerBetCallback(Player player, string bet, int amount)
        {
            try
            {
                FindFirstCasino()
        ...
        public static void PokerBuyChips(Player player, int amount)
        {
            try
            {
                FindFirstCasino()
        ...
        public static void CanceledBetCallback(Player player)
        {
            try
            {
        ...
        public static void PlayAnimCallback(Player player, int flag, string animDictionary, string animName, int x, int y, int z, int angle)
        {
            try
            {
        ...
        public static void StopAnimCallback(Player player)
        {
            try
            {
        ...
        public static void RequestBankCallback(Player player)
        {
            try
            {
                FindFirstCasino()
        ...
        public static void PlayerConnectedCallback(Player player)
        {
            try
            {
                ...
                player.SetData("CURRENTCASINO_ID", casino.BizId);
            }
            catch (Exception e)
            {
        ...
        public static void ChangeImage(Player player, string image)
        {
