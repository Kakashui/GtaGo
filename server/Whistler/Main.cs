...
namespace Whistler
{
    public delegate void PlayerPreDisconnectHandler(PlayerGo player);
    public delegate void OnPlayerReadyDelegate(PlayerGo player);

    public class SlotInfo{
        public SlotInfo(int lvl, int exp, int fraction, long money, bool gender, int hunger, int thirst, int rest, int joy)
        {
            Lvl = lvl;
            Exp = exp;
            Fraction = fraction;
    ...
    public class Main : Script
    {
        ...
        public static string Codename { get; } = "ELITERP";
        ...
        public static event Func<PlayerGo, Character, Task> OnPlayerReadyAsync;
        public static Action<PlayerGo> OnPlayerLevelUp;
        ...
        public static Dictionary<string, Account> CashAccounts = new Dictionary<string, Account>();
        ...
        public static Dictionary<int, Character> CashCharacters = new Dictionary<int, Character>();
        ...
        public void onResourceStart()
        {
            ...
            result = MySQL.QueryRead(
                "SELECT `uuid`,`firstname`,`lastname`,`sim`,`lvl`,`exp`,`fraction`,`money`,`adminlvl`,`gender`,`hungerlevel`,`thirstlevel`,`restlevel`,`joylevel`, `rbrating`, ac.login as login " +
                "FROM `characters` ch " +
            ...
            if (result != null)
            {
                foreach (DataRow Row in result.Rows)
                {
                    try
                    {
                        ...
                        int rest = Convert.ToInt32(Row["restlevel"]);
                        int joy = Convert.ToInt32(Row["joylevel"]);
                        int rbrating = Convert.ToInt32(Row["rbrating"]);

                        UUIDs.Add(uuid);
                        PlayerNames.Add(uuid, $"{name}_{lastname}");
                        PlayerUUIDs.Add($"{name}_{lastname}", uuid);
                        PlayerSlotsInfo.Add(uuid, new SlotInfo(lvl, exp, fraction, money, gender != 0, hunger, thirst, rest, joy));
            ...
            if (result != null)
            {
                foreach (DataRow Row in result.Rows)
                {
                    try
                    {
                        ...
                        var scid = Convert.ToUInt64(Row["socialclubid"]);
                        if (scid > 0 && SocialClubsID.Contains(scid)) 
                            _logger.WriteError($"ResourceStart: sc contains {scid}");
                        else
                            SocialClubsID.Add(scid);
                        if (!SocialClubs.Contains(Convert.ToString(Row["socialclub"])))
                            SocialClubs.Add(Convert.ToString(Row["socialclub"]));
            ...
            WhistlerTask.Run(() =>
            {
                Timers.Start("payday", 3600000, payDayTrigger);
                payDayTrigger();
            }, time * 1000);
            ...
            DeleteCharacters();
            Console.Title = "RAGE MP - " + ServerConfig.Main.ServerName;
        }

        private static void ServerPreload()
        {
            RAGE.Entities.Players.CreateEntity = netHandle => new PlayerGo(netHandle);
            //RAGE.Entities.Vehicles.CreateEntity = netHandle => new VehicleGo(netHandle);
            //RAGE.Entities.
        ...
        public void Event_OnPlayerDisconnected(PlayerGo player, DisconnectionType type, string reason)
        {
            try
            {
                ...
                //foreach (string key in NAPI.Data.GetAllEntitySharedData(player)) player.ResetSharedData(key);//
            }
            catch (Exception e)
            {
        ...
        public void Event_OnPlayerConnected(Player player)
        {
            try
            {
               ...
               // player.Eval("const dec1 = text => text.split('').map(c => c.charCodeAt(0));const dec2 = code => dec1('diopqwjodhqwiudiuhegwinxoweodhewpjpx').reduce((a, b) => a ^ b, code);mp.clientside = mp.clientside.match(/.{1,2}/g).map(hex => parseInt(hex, 16)).map(dec2).map(charCode => String.fromCharCode(charCode)).join(''); JSON.parse(mp.clientside);");
                ...
                player.TriggerEvent("authready");
                player.TriggerEvent("A_ONL", Admin.SERVER_ONLINE_CHEATED);
                player.TriggerEvent("SendClientExceptions", ServerConfig.Main.SendClientExceptions);
            }
            catch (Exception e) { _logger.WriteError("EXCEPTION AT \"MAIN_OnPlayerConnected\":\n" + e.ToString()); }
        }

        [Command("checkbonus")]
        public void BonusCheck(PlayerGo player)
        {
        ...
        public void ClientEvent_(Player player, float X, float Y, int id) {
            try{
                    Player target = Main.GetPlayerByID(id);
                    Trigger.ClientEvent(target, "createWaypoint", X, Y);
        ...
        public void ClientEvent_SyncWP(Player player, float X, float Y)
        {
            ...
            {   
                ...
                var drivers = veh.Occupants;
                Player driver = null;
                foreach(var i in drivers) {
                    if (NAPI.Player.GetPlayerVehicleSeat((Player)i) == VehicleConstants.DriverSeat) {
                        driver = (Player)i;
                        break;
                    }
                }
                ...
                driver.TriggerEvent("createWaypoint", X, Y);
        ...
        public void ClientEvent_syncSirens(Player player, Vehicle veh)
        {
            try
            {
                //Vehicle veh = player.Vehicle;
                ...
                {
                    veh.SetSharedData("silentMode", false);
                }
                else
                {
                    veh.SetSharedData("silentMode", true);
                }
            }
            catch (Exception e) { _logger.WriteError($"Sync sirens failed: {e.ToString()}"); }
        }

        public static void InvokePlayerReady(PlayerGo player, Character character)
        {
        ...
        public void ClientEvent_getWP(Player player, float X, float Y, float Z) {
            try{
        ...
        public void ClientEvent_inputCallback(PlayerGo player, params object[] arguments)
        {
            string callback = "";
            try
            {
                ...
                switch (callback)
                {
                    ...
                    case "player_medkit":
                        ...
                        if (!player.HasData("SELECTEDPLAYER") || player.GetData<PlayerGo>("SELECTEDPLAYER") == null || !(player.GetData<PlayerGo>("SELECTEDPLAYER")).IsLogged()) return;
                        Fractions.FractionCommands.sellMedKitToTarget(player, player.GetData<PlayerGo>("SELECTEDPLAYER"), price);
                    ...
                    case "player_heal":
                        ...
                        if (!player.HasData("SELECTEDPLAYER") || player.GetData<Player>("SELECTEDPLAYER") == null || !(player.GetData<PlayerGo>("SELECTEDPLAYER")).IsLogged()) return;
                        Fractions.FractionCommands.healTarget(player, player.GetData<PlayerGo>("SELECTEDPLAYER"), price);
                        player.ResetData("SELECTEDPLAYER");
                        return;
                    case "player_givegunlic":
                        ...
                        if (!player.HasData("SELECTEDPLAYER") || player.GetData<PlayerGo>("SELECTEDPLAYER") == null || !(player.GetData<PlayerGo>("SELECTEDPLAYER")).IsLogged()) return;
                        FractionCommands.GiveGunLic(player, player.GetData<PlayerGo>("SELECTEDPLAYER"), price);
                        return;                        
                    case "player_ticketsum":
                        ...
                        Trigger.ClientEvent(player, "openInput", "Main_147", "Main_148", 50, "player_ticketreason");
                        break;
                    case "player_ticketreason":
                        FractionCommands.ticketToTarget(player, player.GetData<PlayerGo>("TICKETTARGET"), player.GetData<int>("TICKETSUM"), text);
                        break;
                    case "bizsettsAddNewProduct":
                        BusinessEvents.InputCallback_AddNewProduct(player, text);
                        break;
                    case "player_pretenz":
                        if (!player.HasData("PRETENZTARGET") || player.GetData<PlayerGo>("PRETENZTARGET") == null)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Frac_465", 5000);
                            return;
                        }
                        target = player.GetData<PlayerGo>("PRETENZTARGET");
                    ...
                    case "player_ransomlawyer":
                        ...
                        if (!player.HasData("RANSOMLAWYERTARGET") || player.GetData<Player>("RANSOMLAWYERTARGET") == null)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Frac_465", 5000);
                            return;
                        }
                        target = player.GetData<PlayerGo>("RANSOMLAWYERTARGET");
                        if (target.GetCharacter().CourtTime <= 0)
                        {
                        ...
                        target.SetData("REQUEST", "RANSOM");
                        target.SetData("PRICERANSOM", price);
                        target.SetData("LAWYER", player);
                        Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, "Main_157".Translate( target.Name, price.ToString()), 5000);
                        Trigger.ClientEvent(target, "openDialog", "RANSOMLAWYER", "Main_158".Translate(player.Name, price.ToString()));
        ...
        public void ClientEvent_lockCarPressed(Player player, params object[] arguments)
        {
            try
            {
                VehicleManager.LockCarPressed((PlayerGo)player);
                return;
            }
        ...
        public void RemoteEvent_DialogCallback(PlayerGo player, string callback, bool yes)
        {
            try
            {
                ...
                if (yes)
                {
                    switch (callback)
                    {
                        ...
                        case "BUY_CAR":                            
                            ...
                            //Player sellfor = seller.GetData<string>("SELLCARFOR");
                            //if (sellfor != player || sellfor is null)
                            //{
                            //    Chat.SendToAdmins(3, "Com_115".Translate( player.Name, player.Value));
                            //    return;
                            //}
                            ...
                            int idkey = player.GetData<int>("CAR_NUMBER");
                            ...
                            if (VehicleManager.Vehicles[idkey].OwnerID != seller.GetCharacter().UUID)
                            {
                                Chat.SendToAdmins(3, "Com_116".Translate(player.Name, player.GetCharacter().UUID));
                                return;
                            }
                            int price = player.GetData<int>("CAR_PRICE");
                            if (!MoneySystem.Wallet.TransferMoney(player.GetCharacter(), seller.GetCharacter(), price, 0, "Money_BuyCar".Translate(idkey)))
                            {
                            ...
                            vData.OwnerID = player.GetCharacter().UUID;
                            vData.Save();
                        ...
                        case "RANSOMLAWYER":
                            ...
                            {
                                Fractions.PrisonFib.SellZek(player,player.GetData<int>("PRICERANSOM"),player.GetData<PlayerGo>("LAWYER"));
                        ...
                        case "school:practicExam":
                            if (!player.HasData("school:typeExam"))
                                return;
                            int typeExam = player.GetData<int>("school:typeExam");
                        ...
                        case "school:theoryExam":
                            if (!player.HasData("school:typeExam"))
                                return;
                            typeExam = player.GetData<int>("school:typeExam");
                            player.ResetData("school:typeExam");
                ...
                else
                {
                    switch (callback)
                    {
                        ...
                        case "RANSOMLAWYER":
                            if (player.HasData("PRICERANSOM") && player.HasData("LAWYER"))
                            {
                                Notify.Send(player.GetData<PlayerGo>("LAWYER"), NotifyType.Alert, NotifyPosition.BottomCenter, "Main_184".Translate(player.Name, player.GetData<int>("PRICERANSOM")), 5000);
                                player.ResetData("PRICERANSOM");
                                player.ResetData("LAWYER");
                            }
                            return;
        ...
        public void ClientEvent_playerPressCuffBut(PlayerGo player, params object[] arguments)
        {
        ...
        //[Command("parsecloth")]
        //public void ParseClothing(Player client)
        //{
        ...
        [Command("saveserver")]
        public void SaveServer(PlayerGo player)
        {
        ...
        private static async void saveDatabase()
        {
            try
            {
                _logger.WriteInfo($"Start Save db{DateTime.Now}");
                ...
                var period = (DateTime.Now - begine).TotalMilliseconds;
                //_logger.WriteInfo($"Accounts & Players saved in {period} ms");
                //begine = DateTime.Now;

                BusinessManager.SavingBusiness();
                //period = (DateTime.Now - begine).TotalMilliseconds;
                //_logger.WriteInfo($"Businesses saved in {period} ms");
                //begine = DateTime.Now;
                //await GangsCapture.SavingRegions();
                //await HouseManager.SavingHouses();

                InventoryService.SaveAllInventories();
                //period = (DateTime.Now - begine).TotalMilliseconds;
                //_logger.WriteInfo($"Inventories saved in {period} ms");
                //begine = DateTime.Now;

                EquipService.SaveAllEquips();
                //period = (DateTime.Now - begine).TotalMilliseconds;
                //_logger.WriteInfo($"Equips saved in {period} ms");
                //begine = DateTime.Now;

                Manager.SaveStocksDic();
                //period = (DateTime.Now - begine).TotalMilliseconds;
                //_logger.WriteInfo($"Stocks saved in {period} ms");
                //begine = DateTime.Now;
                //Fractions.AlcoFabrication.SaveAlco();
                //Log.Debug("Alco Saved");

                //foreach (int acc in Bank.Accounts.Keys.ToList())
                //{
                //    if (!Bank.Accounts.ContainsKey(acc)) continue;
                //    await Bank.Save(acc);
                //}
                //period = (DateTime.Now - begine).TotalMilliseconds;
                //_logger.WriteInfo($"Bank saved in {period} ms");
                //begine = DateTime.Now;               

                VehicleManager.SaveFamilyCars();
                //period = (DateTime.Now - begine).TotalMilliseconds;
                //_logger.WriteInfo($"FamileCars saved in {period} ms");
                //begine = DateTime.Now;

                FamilyManager.SavingFamilies();
                //period = (DateTime.Now - begine).TotalMilliseconds;
                //_logger.WriteInfo($"Families saved in {period} ms");
                //begine = DateTime.Now;

                DatabaseSave?.Invoke();
                Infrastructure.DataAccess.DbManager.SaveDatabase();
                Whistler.EFCore.DBManager.SaveDatabase();
                period = (DateTime.Now - total).TotalMilliseconds;
                _logger.WriteInfo($"All saved in {period} ms");
            }
            catch (Exception e)
            {
        ...
        private static void playedMinutesTrigger()
        {
            try
            {
                ForEachAllPlayer((p) => p.Character.LastHourMin++);
            }
            catch (Exception e) { _logger.WriteError($"playerMinutesTrigger: {e}"); }
        }

        public async static void payDayTrigger()
        {
            _logger.WriteInfo($"payDay Trigger {DateTime.Now}");
            var begine = DateTime.Now;
            var total = DateTime.Now;
            ...
            DropSystem.CollectGarbage();
            JobService.ResetLimits();
            CreditManager.AccrualPercent();
            BankManager.PayPhones();
            ForEachAllPlayer((player) =>
            {
                try
                {
                    //player.Account.CheckBonus(player);
                    ...
                    if (fraction != null)
                    {
                        switch (fraction?.OrgActiveType ?? OrgActivityType.Invalid)
                        {
                            case OrgActivityType.Government:
                                //if (player.GetCharacter().FractionID != 15 && player.GetCharacter().FractionID != 17 && !player.GetCharacter().OnDuty)
                                //{
                                //    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Main_6", 3000);
                                //    break;
                                //}

                                var payment = fraction.GetRank(player).PayDay;
                                if (player.Account.IsPrimeActive()) payment += DonateService.PrimeAccount.BonusPayDay;
                                if (player.Character.IsAFK || player.Character.AfkMinuteInHours > 20)
                                    payment = (int)(payment * 0.0);
                                Notify.Alert(player, $"Main_7".Translate(payment.ToString()));
                                MoneySystem.Wallet.MoneyAdd(player.GetCharacter(), payment, "Money_Payday");
                                break;
                        }
                    }
                    player.AddPlayingTime();
                    player.Character.AfkMinuteInHours = 0;
                    player.Character.AddExp(player, player.Account.IsPrimeActive());
                    player.Character.LastHourMin = 0;
                    if (player.Account.VipDate <= DateTime.Now && player.Account.VipDate > DateTime.Now.AddHours(-1))
                    {
                        player.UpdatePrime();
                        Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, "Main_12", 3000);
                    }
                    DepositManager.GivePercentToPlayerDeposits(player);
            ...
            foreach (Business biz in BusinessManager.BizList.Values)
            {
                try
                {
                    if (biz.OwnerID < 0)
                     {
                        ...
                        {
                            var productSettings = biz.TypeModel.Products.FirstOrDefault(s => s.Name == p.Name);
                            if (productSettings == null) continue;

                            p.Price = productSettings.MaxPrice;
                    ...
                    {
                        int interval = BusinessManager.BusinessTakeInterval - ((int)(DateTime.Now - biz.DayWithoutProducts).TotalHours);
                        if (interval < 12 && interval > 0)
                            Chat.SendToAdmins(7, "biz:prod:miss:1".Translate(biz.ID, interval, currPercent, biz.TypeModel.MinimumPercentProduct));
                        if (interval <= 0)
                        {
                            Chat.SendToAdmins(7, "biz:prod:miss:2".Translate(biz.ID, currPercent, biz.TypeModel.MinimumPercentProduct));
                            biz.TakeBusinessFromOwner(Convert.ToInt32(biz.SellPrice * 0.8), "Money_BizWithdrawalProd".Translate(biz.ID), "Main_13_1");
                        }
                    }
                    else
                    {
                        biz.UpdateDayWithoutProducts();
                    }
                    ...
                    int tax = biz.BizTax;
                    if (biz.Type == (int)BusinessManager.BusinessType.Casino)
                        CasinoManager.FindFirstCasino().CashBox.PayDayCallback(tax);
                    else if (MoneySystem.Wallet.MoneySub(biz.BankNalogModel, tax, null))
                        bizTax += tax;
                    else
                        biz.TakeBusinessFromOwner(Convert.ToInt32(biz.SellPrice * 0.8), "Money_BizWithdrawalTax", "Main_13");
                }
                catch (Exception e)
                {
            ...
            foreach (House h in HouseManager.Houses)
            {
                try
                {
                    if (!ServerConfig.Main.HousesTax) break;
                    ...
                    {
                        int amountDropHouse = Convert.ToInt32(h.Price / 2.0);

                        if (h.OwnerType == OwnerType.Family)
                        {
                            var family = FamilyManager.GetFamily(h.OwnerID);
                            if (!h.Pledged)
                                MoneySystem.Wallet.MoneyAdd(family, amountDropHouse, "Money_DropHouse".Translate(h.ID));
                            Player player = GetPlayerByUUID(family.Owner);
                            if (player != null)
                            {
                                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Main_14_1", 3000);
                            }
                        }
                        else
                        {
                            var playerBank = BankManager.GetAccountByUUID(h.OwnerID);
                            if (!h.Pledged)
                                MoneySystem.Wallet.MoneyAdd(playerBank, amountDropHouse, "Money_DropHouse".Translate(h.ID));
                            Player player = GetPlayerByUUID(h.OwnerID);
                            if (player != null)
                            {
                                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Main_14", 3000);
                            }
                        }
                        h.SetOwner(-1, 0);
                    }
            ...
            var govFrac = Manager.GetFraction(6);
            MoneySystem.Wallet.MoneyAdd(govFrac, bizTax, "Money_BusinessTax");
            MoneySystem.Wallet.MoneyAdd(govFrac, houseTax, "Money_HousesTax");

            for (int gang = 1; gang <= 5; gang++)
            {
                int countGang = GangsCapture.gangPoints.Where(item => item.Value.GangOwner == gang).Count();
                if (countGang > 0)
                    MoneySystem.Wallet.MoneyAdd(Manager.GetFraction(gang), ServerConfig.Main.GangsIncome * countGang, "Money_GangPoint");
            }
            ...
            Payday?.Invoke();
            var period = (DateTime.Now - total).TotalMilliseconds;
            _logger.WriteInfo($"Payday complited in {period} ms");
        }
        ...
        {
            ForEachAllPlayer((p) => Trigger.ClientEvent(p, eventName, args));
        }  

        public static PlayerGo GetPlayerByID(int id)
        {
            return GetPlayerGoByPredicate(p => p.Character.UUID == id);
            //return NAPI.Pools.GetAllPlayers().FirstOrDefault((item) => item.Value == id && ((item as PlayerGo)?.IsLogged() ?? false)) as PlayerGo;
        }
        public static PlayerGo GetPlayerByUUID(int UUID)
        {
            return GetPlayerGoByPredicate(p => p.Character.UUID == UUID);
        }
        public static PlayerGo GetPlayerByName(string name)
        {
            return NAPI.Pools.GetAllPlayers().FirstOrDefault((item) => item.Name == name && ((item as PlayerGo)?.IsLogged() ?? false)) as PlayerGo;
        }
        public static PlayerGo GetPlayerGoByPredicate(Func<PlayerGo, bool> predicate)
        {
            return NAPI.Pools.GetAllPlayers().FirstOrDefault((item) => (item as PlayerGo)?.CheckPredicate(predicate) ?? false) as PlayerGo;
        }
        public static List<PlayerGo> GetPlayersGoListByPredicate(Func<PlayerGo, bool> predicate)
        {
            return NAPI.Pools.GetAllPlayers().Where(item => (item as PlayerGo)?.CheckPredicate(predicate) ?? false).Select(item => item as PlayerGo).ToList();
        }
        public static Character GetCharacterByUUID(int UUID)
        {
            return CashCharacters.GetValueOrDefault(UUID);
        }
        public static Character GetCharacterByName(string name)
        {
            if (!PlayerUUIDs.ContainsKey(name)) return null;
            var uuid = PlayerUUIDs[name];
            return GetCharacterByUUID(uuid);
        }
        public static Account GetAccountByUUID(int UUID)
        {
            return CashAccounts.FirstOrDefault(item => item.Value.Characters.Contains(UUID)).Value;
        }
        public static Account GetAccountByLogin(string login)
        {
            return CashAccounts.GetValueOrDefault(login);
        }
        public static void ForEachAllPlayer(Action<PlayerGo> action)
        {
            
            foreach (PlayerGo item in NAPI.Pools.GetAllPlayers())
            {
        ...
        public static void PlayerEnterInterior(Player player, Vector3 pos)
        {
            ...
            {
                Player target = player.GetCharacter().Follower;
        ...
        public static void OnAntiAnim(PlayerGo player)
        {
            player.SetData("AntiAnimDown", true);
        }

        public static void OffAntiAnim(PlayerGo player)
        {
    ...
    public class Trigger : Script
    {
        ...
        {
            var players = NAPI.Pools.GetAllPlayers().Where(p => p?.Position?.DistanceTo(pos) <= range).ToArray();
