...
namespace Whistler.Inventory.Models
{
    ...
    {
        public RepairKit() : base() { }
        public RepairKit(ItemNames name, bool promo, bool temporary) : base(name, 1, promo, temporary)
        {

        }

        private static WhistlerLogger _logger = new WhistlerLogger(typeof(Other));
        private static RepairKitConfig DefaultConfig(ItemNames name)
        {
        ...
        [JsonIgnore]
        public RepairKitConfig Config { get { return (!RepairKitConfigs.Config.ContainsKey(Name)) ? DefaultConfig(Name) : RepairKitConfigs.Config[Name]; } }
