...
namespace Whistler.Inventory.Models
{
    public class Equip
    {
        ...
        public void CorrectClothes(Player player)
        {
        ...
        public void CorrectArmor(Player player)
        {
        ...
        public void Sync()
        {
            ...
            WhistlerTask.Run(() =>
            {
                _owner.TriggerEvent("inv:clear:equip");
                foreach (var cloth in Clothes)
                {
                    if (cloth.Value != null)
                    {
                        _owner.TriggerEvent("inv:update:equip", (int)ItemTypes.Clothes, (int)cloth.Key, cloth.Value.GetItemData());
                    }
                }
                ...
                foreach (var weapon in Weapons)
                {
                    if (weapon.Value != null)
                    {
                        _owner.TriggerEvent("inv:update:equip", (int)ItemTypes.Weapon, (int)weapon.Key, weapon.Value.GetItemData());
                    }
                }
            }, 1000);
        }
        ...
        public void Update(ClothesSlots slot)
        {
            ...
            var clothes = Clothes[slot];
            WhistlerTask.Run(() =>
            {              
                if (clothes == null)
                    InventoryService.DefaultClothes[slot].Set(_owner);
                else
                    clothes.Equip(_owner);
                _owner.TriggerEvent("inv:update:equip", (int)ItemTypes.Clothes, (int)slot, clothes?.GetItemData());
            });
        }
        ...
        public void Update(WeaponSlots slot)
        {
            ...
            WhistlerTask.Run(() =>
            {
                ...
                else
                {
                    if (CurrentWeapon == slot)
                    {
                        _owner.GiveWeapon((WeaponHash)weapon.Config.WeaponHash, 0);
                ...
                _owner.TriggerEvent("inv:update:equip", (int)ItemTypes.Weapon, (int)slot, weapon?.GetItemData());
            });
        }

        internal void DeleteEmptyArmor(Player player = null)
        {
        ...
        public void ReloadWeapon(Player player, int ammo)
        {
            if (CurrentWeapon == WeaponSlots.Invalid || Weapons[CurrentWeapon] == null)
            {
                Update(true);
                return;
            }
            if (Weapons[CurrentWeapon].Ammo < ammo || Weapons[CurrentWeapon].MaxAmmo == ammo) return;
            var inventory = _owner.GetInventory();
            var ammoItem = inventory.SubItemByName(Weapons[CurrentWeapon].Config.AmmoType, Weapons[CurrentWeapon].MaxAmmo - ammo, LogAction.None);
            if (ammoItem == null) Weapons[CurrentWeapon].Ammo = ammo;
            else
            {
                Weapons[CurrentWeapon].Ammo = ammoItem.Count + ammo;
            }
            _changed = true;
            player.TriggerEvent("weapon:reload", Weapons[CurrentWeapon].Ammo);
        }
                
        //"attach:weapon:1", "attach:weapon:2", "attach:weapon:3", "attach:weapon:4", "weapon:current"

        public void SetActiveItem(Player player, WeaponSlots slotId, int ammo)
        {
            if (slotId == WeaponSlots.Invalid) return;
            if (Weapons[slotId] == null) return;

            if (CurrentWeapon == WeaponSlots.Invalid)
            {
                player.GiveWeapon((WeaponHash)Weapons[slotId].Config.WeaponHash, 0);
                player.SetSharedData($"weapon:current", Weapons[slotId].GetWeaponData()); 
                if(false && slotId != WeaponSlots.Weapon3 && slotId != WeaponSlots.Weapon4) player.ResetSharedData($"attach:weapon:{(int)slotId}");
            ...
            else
            {
                ...
                if (CurrentWeapon == slotId)
                {
                    ...
                    player.ResetSharedData("weapon:current");
                    if(false && slotId != WeaponSlots.Weapon3 && slotId != WeaponSlots.Weapon4) player.SetSharedData($"attach:weapon:{(int)slotId}", Weapons[slotId].GetWeaponData());
                    CurrentWeapon = WeaponSlots.Invalid;
                }
                else
                {
                    ...
                    player.GiveWeapon((WeaponHash)Weapons[slotId].Config.WeaponHash, 0);
                    player.SetSharedData($"weapon:current", Weapons[slotId].GetWeaponData());
                    if(false && Weapons[CurrentWeapon] != null && CurrentWeapon != WeaponSlots.Weapon3 && CurrentWeapon != WeaponSlots.Weapon4) player.SetSharedData($"attach:weapon:{(int)CurrentWeapon}", Weapons[CurrentWeapon].GetWeaponData());
                    CurrentWeapon = slotId;
                }
            }
        }

        public bool TryEquipItem(Player player, BaseItem clothes, ClothesSlots slot)
        {
        ...
        public bool TryEquipItem(Player player, BaseItem weapon, WeaponSlots slot)
        {
            if (player != _owner) return false;
        ...
        public bool EquipItem(Player player, ClothesBase clothes, ClothesSlots slot, ref BaseItem oldItem, LogAction log = LogAction.Create)
        {
        ...
        public bool EquipItem(Player player, Weapon weapon, WeaponSlots slot, ref BaseItem oldItem, LogAction log = LogAction.Create)
        {
            ...
            if(CurrentWeapon == slot)
            {
                player.GiveWeapon((WeaponHash)weapon.Config.WeaponHash, 0);
                player.SetSharedData("weapon:current", weapon.GetWeaponData());
            }
            else
            {
                if(false && slot != WeaponSlots.Weapon3 && slot != WeaponSlots.Weapon4) player.SetSharedData($"attach:weapon:{(int)slot}", weapon.GetWeaponData());
            }
            Update(slot);
            _changed = true;
            if (log > LogAction.None)
        ...
        public BaseItem RemoveItem(Player player, ClothesSlots slot, LogAction log = LogAction.Delete)
        {
        ...
        public BaseItem RemoveItem(Player player, WeaponSlots slot, LogAction log = LogAction.Delete)
        {
            if (player != _owner) return null;
        ...
        public void MoveItem(Player player, WeaponSlots from, WeaponSlots to)
        {
        ...
        public void Subscribe(PlayerGo player)
        {
            _owner = player;
            Sync();
        }
        public bool RemoveWeapons(Func<KeyValuePair<WeaponSlots,Weapon>, bool> predicate, Player player = null)
        {
        ...
        public bool RemoveClothes(Func<KeyValuePair<ClothesSlots, ClothesBase>, bool> predicate, LogAction log = LogAction.Delete, Player player = null)
        {
