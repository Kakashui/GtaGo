...
namespace Whistler.Inventory.Models
{
    public class InventoryModel
    {
        ...
        internal bool IsSubscribed(Player player)
        {
        ...
        public void Update(Player player)
        {
            lock (_subscribes)
            {
                if (!_subscribes.Contains(player))
                    ...
                    WhistlerTask.Run(() => player.TriggerEvent("inv:update", Id, GetInventoryData(), MaxWeight, Size));
            }           
        }
        public void Update()
        {
        ...
        private void UpdateItem(BaseItem item)
        {
            WhistlerTask.Run(() =>
            {
                foreach (var player in _subscribes)
                {
                    player.TriggerEvent("inv:update:item", Id, item.GetItemData());
                }
            });
        ...
        private void UpdateItem(int index)
        {
            WhistlerTask.Run(() =>
            {
                foreach (var player in _subscribes)
                {
                    player.TriggerEvent("inv:update:item", Id, new List<int> { 0, 0, index });
                }
            });           
        }  
        ...
        public bool RemoveItems(Func<BaseItem, bool> predicate, LogAction log = LogAction.Delete, Player player = null)
        {
