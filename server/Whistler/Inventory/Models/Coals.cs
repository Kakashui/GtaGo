...
{
    public class Coals : BaseItem
    {
        public Coals() : base() { }
        public Coals(ItemNames name, int count, bool promo, bool temporary) : base(name, count, promo, temporary)
        {

        }

        private static WhistlerLogger _logger = new WhistlerLogger(typeof(Coals));
        private static CoalsConfig DefaultConfig(ItemNames name)
        {
            _logger.WriteWarning($"No config for: {name}");
            return new CoalsConfig
            {
                Weight = 5000,
                CanUse = true,
                Stackable = true,
        ...
        [JsonIgnore]
        public CoalsConfig Config { get { return (!CoalsConfigs.Config.ContainsKey(Name)) ? DefaultConfig(Name) : CoalsConfigs.Config[Name]; } }
        ...
        public override bool Use(PlayerGo player)
        {           
            return true;
        }

        public override bool CanUse(PlayerGo player)
        {
