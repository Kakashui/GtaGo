...
namespace Whistler.Inventory
{
    public class InventoryEvents : Script
    {
        ...
        public static Action<PlayerGo, int, ItemTypes> ItemInteract;
        ...
        public static void WorldObjectInteracted(PlayerGo player, GTANetworkAPI.Object entity)
        {
        ...
        public static void PickupItem(Player player, int itemId, ItemTypes itemType)
        {
            ...
            {
                GameLog.ItemsLog(itemFrom.Id, "0", $"i{player.GetInventory().Id}", itemFrom.Name, itemFrom.Count, itemFrom.GetItemLogData(), LogAction.Move, player.GetCharacter().UUID);
        ...
        public void MoveItem(Player player, int fromId, int fromIndex, int toId, int toIndex, int count)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (!inventoryTo.AddItem(itemFrom, toIndex, LogAction.None))
                    {
                        ...
                        {
                            DropSystem.DropItem(itemFrom, player.Position, player.Dimension);
                            GameLog.ItemsLog(itemFrom.Id, $"i{inventoryFrom.Id}", "0", itemFrom.Name, itemFrom.Count, itemFrom.GetItemLogData(), LogAction.Move, player.GetCharacter()?.UUID ?? -1);
                            Core.Animations.Animations.DropItem(player);
                        }
                    }
                    else
                        GameLog.ItemsLog(itemFrom.Id, $"i{inventoryFrom.Id}", $"i{inventoryTo.Id}", itemFrom.Name, itemFrom.Count, itemFrom.GetItemLogData(), LogAction.Move, player.GetCharacter().UUID);
                }
            }
            catch (Exception ex)
            {
        ...
        public void DropItem(Player player, int fromId, int fromIndex, int count)
        {
            try
            {
                if (!player.IsLogged()) return;
                ...
                {
                    DropSystem.DropItem(itemFrom, player.Position, player.Dimension);
                    GameLog.ItemsLog(itemFrom.Id, $"i{inventoryFrom.Id}", "0", itemFrom.Name, itemFrom.Count, itemFrom.GetItemLogData(), LogAction.Move, player.GetCharacter().UUID);
                    Core.Animations.Animations.DropItem(player);
                }
            }
            catch (Exception ex)
        ...
        public void Useitem(PlayerGo player, int invId, int itemIndex)
        {
            try
            {
                if (!player.IsLogged()) return;
        ...
        public void Useitem(PlayerGo player, int itemIndex)
        {
            try
            {
                if (!player.IsLogged()) return;
        ...
        public void UseItemFast(PlayerGo player, int itemId)
        {
            try
            {
                if (!player.IsLogged()) return;
        ...
        public void GetInventoryById(Player player, int id)
        {
            try
            {
                if (!player.IsLogged()) return;
                if (id == 0)
                {
                    player.TriggerEvent("inv:update", 0, player.GetNearItemsDTO(), -1, 2);
                }
                else
                {
        ...
        public void GetPersonalId(Player player)
        {
            InventoryService.SyncInventoryId(player);
        }

        [RemoteEvent("inv:pickup")]
        public void PickupItem(Player player, int itemId, int toId, int toIndex, int count)
        {
            try
            {
                if (!player.IsLogged()) return;
                ...
                {
                    GameLog.ItemsLog(itemFrom.Id, "0", $"i{inventoryFrom.Id}", itemFrom.Name, itemFrom.Count, itemFrom.GetItemLogData(), LogAction.Move, player.GetCharacter().UUID);
                    Core.Animations.Animations.PickUpItem(player);
                }
        ...
        public void CloseInventory(Player player, int stockId)
        {
            try
            {
                if (!player.IsLogged()) return;
        ...
        [Command("spmw")]
        public void SetPlayerWeight(Player player, int playerId, int val)
        {
            try
            {
