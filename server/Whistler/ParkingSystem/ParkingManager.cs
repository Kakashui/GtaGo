...
namespace Whistler.ParkingSystem
{
    class ParkingManager : Script
    {
        ...
        public static void interactPressed(Player player, int id)
        {
            try
            {
                DialogUI.Open(player, "Main_173".Translate(ParkingTariff), new List<DialogUI.ButtonSetting>
                {
                    new DialogUI.ButtonSetting
                    {
                        ...
                        Action = p =>
                        {
                            ...
                            if (par == null)
                                return;
                            if (player.GetCharacter().ParkingSpawnCar > 0)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Park_1", 3000);
                                return;
                            }
                            if (HouseManager.GetHouse(player, true) != null)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Park_2", 3000);
                                return;
                            }
                            var vehicleID = VehicleManager.getAllHolderVehicles(player.GetCharacter().UUID, OwnerType.Personal).FirstOrDefault();
                            if (!VehicleManager.Vehicles.ContainsKey(vehicleID))
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Park_4", 3000);
                                return;
                            }
                            if (!Wallet.TransferMoney(player.GetCharacter(), Manager.GetFraction(6), ParkingTariff, 0, "Money_Parking"))
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Park_5", 3000);
                                return;
                            }

                            var position = par.Positions.Dequeue();
                            par.Positions.Enqueue(position);
                            var veh = VehicleManager.Vehicles[vehicleID].Spawn(position.Position + new Vector3(0, 0, 0.3), position.Rotation, 0);

                            player.GetCharacter().ParkingSpawnCar = vehicleID;
                        }
                    },
        ...
        public static void DeleteParkingVehicle(Player player)
        {
            if (!player.IsLogged())
                return;
            if (player.GetCharacter().ParkingSpawnCar > 0)
            {
                VehicleManager.GetVehicleByUUID(player.GetCharacter().ParkingSpawnCar)?.CustomDelete();
                player.GetCharacter().ParkingSpawnCar = -1;
            }
        }
    }
