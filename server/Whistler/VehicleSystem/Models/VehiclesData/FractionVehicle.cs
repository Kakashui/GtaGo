...
namespace Whistler.VehicleSystem.Models.VehiclesData
{
    class FractionVehicle : VehicleBase
    {
        ...
        protected override Vehicle SpawnCar(Vector3 position, Vector3 rotation, uint dimension)
        {
            ...
            var model = (VehicleHash)NAPI.Util.GetHashKey(ModelName);
            if (model == VehicleHash.Pbus || model == VehicleHash.Pranger || model == VehicleHash.Volatus)
            {
                if (model == VehicleHash.Volatus)
        ...
        public override bool CanAccessVehicle(Player player, AccessType access)
        {
            ...
            switch (access)
            {
                case AccessType.OpenTrunk:
                    ...
                    {
                        if (player.GetCharacter().FractionLVL < MinRank)
                            return false;
                        else
                            return true;
                    }
                ...
                case AccessType.EngineChange:
                    ...
                    /*if (CanConvoy != ConvoyType.None && OwnerID == 7 && fracid == 18)
                        if ((player.GetCharacter().FractionLVL < 4 && CanConvoy == ConvoyType.Vehicle) || (player.GetCharacter().FractionLVL == 3 && CanConvoy == ConvoyType.Helicopter))
                            return true;*/
                    if (player.GetCharacter().AdminLVL >= 1)
                        return true;
                    if (fracid == OwnerID && player.GetCharacter().FractionLVL >= MinRank)
                        return true;
        ...
        {
            DestroyVehicle();
            WhistlerTask.Run(() => { Spawn(); }, 1500);            
        }

        public void SpawnSpike(Vehicle vehicle)
        {
        ...
        private void OnEntityEnterColShape(ColShape colShape, Player client)
        {
            ...
            {
                client.TriggerEvent("veh:setTyreBurst");
            }
        }
        ...
        protected override void PlayerEnterVehicle(PlayerGo player, Vehicle vehicle, VehicleGo vehGo, sbyte seatid)
        {
            ...
            {
                var fracid = player.GetCharacter().FractionID;
                //if (player.GetCharacter().AdminLVL >= 1) return;
                ...
                //}
                //if (Holder != fracid || player.GetCharacter().FractionLVL < MinRank)
                //{
