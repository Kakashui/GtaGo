...
namespace Whistler.VehicleSystem.Models.VehiclesData
{
    class JobVehicle : VehicleBase
    {
        ...
        protected override Vehicle SpawnCar(Vector3 position, Vector3 rotation, uint dimension)
        {
            ...
            Fuel = VehicleConfiguration.GetConfig(vehicle.Model).MaxFuel;
            ...
            VehicleStreaming.SetVehicleFuel(vehicle, VehicleConfiguration.GetConfig(vehicle.Model).MaxFuel);
            OnWork = false;
        ...
        protected override void PlayerEnterVehicle(PlayerGo player, Vehicle vehicle, VehicleGo vehGo, sbyte seatid)
        {
            ...
            if (player.VehicleSeat == VehicleConstants.DriverSeat)
            {
                ...
                if (((JobVehicle)vehGo.Data).WorkType == WorkType.DockLoader)
                {
                    if (player.GetCharacter().WorkID != (int)WorkType.DockLoader)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "DockLoader_1".Translate(), 3000);
                        VehicleManager.WarpPlayerOutOfVehicle(player);
                        return;
                    }
                    if (BusinessManager.GetBusinessByOwner(player.GetCharacter().UUID) != null)
                    {
