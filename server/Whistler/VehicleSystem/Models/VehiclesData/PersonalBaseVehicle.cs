...
namespace Whistler.VehicleSystem.Models.VehiclesData
{
    abstract public class PersonalBaseVehicle : VehicleBase, IWhistlerProperty
    {
        ...
        {          
            var vehHash = NAPI.Util.GetHashKey(ModelName);
            Vehicle vehicle = NAPI.Vehicle.CreateVehicle(vehHash, position, rotation.Z, 0, 0, dimension: dimension);
        ...
        public override void VehicleDeath(Vehicle vehicle)
        {
            if (OwnerType == OwnerType.Personal)
            {
                ...
                Player owner = NAPI.Player.GetPlayerFromName(ownerName);
                if (owner != null)
