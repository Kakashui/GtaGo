    ...
    public class PersonalVehicle : PersonalBaseVehicle
    {
        ...
        public override bool CanAccessVehicle(Player player, AccessType access)
        {
            ...
            switch (access)
            {
                ...
                case AccessType.SellDollars:
                    return OwnerID == player.GetCharacter().UUID && PropertyBuyStatus == PropBuyStatus.Bought && !Pledged;
                case AccessType.SellRouletteCar:
                    return OwnerID == player.GetCharacter().UUID && PropertyBuyStatus == PropBuyStatus.Roulette && !Pledged;
                case AccessType.SellZero:
                    return OwnerID == player.GetCharacter().UUID && PropertyBuyStatus == PropBuyStatus.Given && !Pledged;
                default:
                    return false;
            }
        }
        ...
        public override void DestroyVehicle()
        {
            ...
            if (vehicle != null)
            {
                ...
                if (player.IsLogged() && player.GetCharacter().ParkingSpawnCar == ID)
                {
