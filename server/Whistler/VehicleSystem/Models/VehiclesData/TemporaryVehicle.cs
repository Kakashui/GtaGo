...
namespace Whistler.VehicleSystem.Models.VehiclesData
{
    class TemporaryVehicle : VehicleBase
    {
        ...
        public TemporaryVehicle(uint modelName, string numberPlate, VehicleAccess accessType, Player driver) : base()
        {
            ID = _id--;
            Access = accessType;
            Number = numberPlate;
        ...
        public override bool CanAccessVehicle(Player player, AccessType access)
        {
            ...
            switch (Access)
            {
                ...
                case VehicleAccess.MedKits:
                    if (player.GetCharacter().AdminLVL >= 1)
                        return true;
                    if (player.GetCharacter().MediaHelper > 0)
                        return true;
                    return false;
        ...
        public override void RespawnVehicle()
        {
            switch (Access)
            {
                case VehicleAccess.WorkBus:
                    ...
                    if (Driver.IsLogged())
                    {
                        Driver.SetData("ON_WORK", false);
                        Driver.GetCharacter().WorkID = 0;
                        Notify.Send(Driver, NotifyType.Info, NotifyPosition.BottomCenter, $"Jobs_165", 3000);
                        Trigger.ClientEvent(Driver, "bus:clear");
        ...
        protected override void PlayerEnterVehicle(PlayerGo player, Vehicle vehicle, VehicleGo vehGo, sbyte seatid)
        {
            if (player.VehicleSeat == VehicleConstants.DriverSeat)
            {
                ...
                switch (Access)
                {
                    ...
                    case VehicleAccess.WorkTruck:
                        ...
                        player.TriggerEvent("truckers:destroyMarker");
                    ...
                    case VehicleAccess.WorkBus:
                        ...
                        player.TriggerEvent("bus:destroyMarker");
                        ...
                        break;
                    case VehicleAccess.DockLoader:
                        player.TriggerEvent("dockLoader:destroyMarker");
                        break;
                    case VehicleAccess.Admin:
                    case VehicleAccess.Work:
                    case VehicleAccess.School:
            ...
            else
            {
                ...
                {
                    case VehicleAccess.WorkBus:
                        if ((Driver?.GetData<bool>("IN_WORK_CAR") ?? false) == true)
                        {
                            var price = 30;
                            if (Wallet.TransferMoney(player.GetCharacter(), Manager.GetFraction(6), price, 0, "Money_BusTicket"))
                            {
            ...
            if (Driver == player)
            {
                switch (Access)
                {
                    ...
                    case VehicleAccess.WorkTruck:
                        ...
                        break;
                    case VehicleAccess.WorkBus:
                        if (player.GetCharacter().WorkID == 4 && player.GetData<bool>("ON_WORK"))
                        {
                            Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Jobs_164", 3000);
