...
namespace Whistler.VehicleSystem
{
    class VehicleOperations
    {
        ...
        public static bool CheckCorrectVehiclePrice(Player player, string model, int price, bool notify = true)
        {
        ...
        public static void SellVeh(Player player, int vehId)
        {
            var vData = VehicleManager.GetVehicleBaseByUUID(vehId) as PersonalBaseVehicle;
        ...
        private static void SellVehForMoney(Player player, PersonalBaseVehicle vData, int price)
        {
            price = VehicleConstants.GetCorrectSalePrice(price, player.GetAccount().IsPrimeActive());
                    ...
                    Action = (p) =>
                    {
                        switch (vData.OwnerType)
                        {
                            case OwnerType.Personal:
                                MoneySystem.Wallet.MoneyAdd(player.GetCharacter(), price, "Money_SellCar".Translate(vData.ID));
                                break;
                            case OwnerType.Family:
                                MoneySystem.Wallet.MoneyAdd(player.GetFamily(), price, "Money_SellCar".Translate(vData.ID));
                                break;
                            default:
                                return;
                        }
        ...
        private static void SellVehForDonate(Player player, PersonalBaseVehicle vData)
        {
            try
            {
                if (vData.OwnerID != player.GetCharacter().UUID)
                    return;
