...
namespace Whistler.Customization
{
    ...
    public static class CustomizationService
    {
        ...
        {
            var qp = new QuestPed(PedHash.Doctor01SMM, new Vector3(321.8153, -565.7399, 43.1297), "House", "qp:role:doc", 150);
            qp.PlayerInteracted += Qp_PlayerInteracted;
        }

        private static void Qp_PlayerInteracted(PlayerGo player, QuestPed ped)
        {
            var InfoTab = new DialogPage("qp:tattoo:doc:8".Translate(PlasticSurgeryPrice),
                ped.Name,
                ped.Role);

            var startPage = new DialogPage("qp:tattoo:doc:1",
                ped.Name,
                ped.Role)
                .AddAnswer("qp:tattoo:doc:2", SendToCreatorFromDoctor)
                .AddAnswer("qp:tattoo:doc:3", OpenRemoveTattooMenu)
                .AddAnswer("qp:tattoo:doc:4", InfoTab)
                .AddCloseAnswer("qp:tattoo:doc:5");

            InfoTab.AddAnswer("qp:tattoo:doc:6", startPage)
                .AddCloseAnswer("qp:tattoo:doc:7");

            startPage.OpenForPlayer(player);
        }

        private static Vector3 _removeTattooPosition = new Vector3(323.8332, -582.7084, 43.12974);
        private static void OpenRemoveTattooMenu(PlayerGo player)
        {
            ...
            player.TriggerEvent("tattoo:remove:open");
        }

        public static void SendToCreatorFromDoctor(PlayerGo player)
        {
            var half = PlasticSurgeryPrice / 2;
            if (Wallet.TransferMoney(player.GetCharacter(), new List<(IMoneyOwner, int)> 
            { 
                (Manager.GetFraction(6), half), 
                (Manager.GetFraction(8), half) 
            }, "Money_PlasticSurgery"))
            {
                SendToCreator(player, -1);
            }
            else Notify.SendError(player, "tattoo:remove:nomany".Translate(PlasticSurgeryPrice));
        }
        ...
        )
        {
            ...
            Trigger.ClientEvent(player, "customization:destroy");
        ...
        internal static bool ConvertOldCustomization(Player player)
        {
        ...
        public static void SendToCreator(Player player, int slot)
        {
            ...
            player.Dimension = Convert.ToUInt32(DimensionID++);
            player.Rotation = new Vector3(0f, 0f, CreatorCharRot);
            player.Position = CreatorCharPos;


            Trigger.ClientEvent(player, "customization:create", slot);
        }

        private static void SendPlayerToWorld(Player player)
        {
            Trigger.ClientEvent(player, "customization:destroy");
            Vector3 pos = player.HasData("Creator_PrevPos") ? player.GetData<Vector3>("Creator_PrevPos") : new Vector3(-1037.248, -2731.725, 13.75665);
