...
namespace Whistler.Customization
{
    class CustomizationEvents : Script
    {
        ...
        public void ApplayCharacter(Player player)
        {
        ...
        )
        {
            try
            {
                if (player.GetPlayerGo() == null) return;
                string name="";
                if(slot >= 0)
                {
                    if (!Character.NameIsCorrect(firstName, lastName))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Core_79", 3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Core_80", 3000);
                        return;
                    }
                }
