...
namespace Whistler.MP.Arena.UI
{
    internal class ArenaUiUpdateHandler : Script
    {
        ...
        public static void OpenMenuForPlayer(Player player)
        {
            ...
            player.TriggerEvent("ARENA::OPEN::GUI::SERVER", JsonConvert.SerializeObject(list));
        }
        
        public static void SubsribePlayer(Player player)
        {
            if (!Subscribers.Contains(player))
                Subscribers.Add(player);
        }

        public static void UnsubscribePlayer(Player player)
        {
            if (Subscribers.Contains(player))
        ...
        private static void OnBattleCreated(IArenaBattle battle)
        {
            ...
            Subscribers.ForEach(s => s.TriggerEvent("ARENA:DEACTIVATE::LOBBY::SERVER", battle.Lobby.Id));
            
            // battle.Members.ForEach(m => m.Player.TriggerEvent("arena:battle:start", 
        ...
        {    
            ...
            Subscribers.ForEach(s => s.TriggerEvent("ARENA::PLAYER::SWITCH::TEAM::SERVER", 
        ...
        private static void OnLobbyLeaved(IArenaLobbyMember member)
        {
            ...
            Subscribers.ForEach(s => s.TriggerEvent("ARENA::LOBBY::LEAVE::SERVER", 
                JsonConvert.SerializeObject(mapper.Map<LobbyMemberDTO>(member))));
        }

        private static void OnLobbyDeleted(IArenaLobby lobby)
        {
            Subscribers.ForEach(s => s.TriggerEvent("ARENA::REMOVE::LOBBY::SERVER", lobby.Id));
        }
        ...
        private static void OnLobbyJoined(IArenaLobbyMember member)
        {
            ...
            Subscribers.ForEach(s => s.TriggerEvent("ARENA::JOIN::LOBBY::SERVER", 
                JsonConvert.SerializeObject(mapper.Map<LobbyMemberDTO>(member))));
        }
        ...
        private static void OnLobbyCreated(IArenaLobby lobby)
        {
            ...
            switch (lobby)
            {
                case StrikeLobby concreteStrikeLobby:
                    Subscribers.ForEach(s => s.TriggerEvent("ARENA::ADD::LOBBY::SERVER", 
                        JsonConvert.SerializeObject(mapper.Map<StrikeLobbyDTO>(concreteStrikeLobby))));
                    Trigger.ClientEvent(lobby.Leader.Player, "ARENA::OPEN::LOBBY::SERVER", lobby.Id);
                    break;
