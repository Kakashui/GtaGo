...
namespace Whistler.MP.OrgBattle.BattleModels
{
    class OrgBattleWithPoints : BattleBase
    {
        ...
        internal override bool PlayerDeath(Player player, Player killer, uint weapon)
        {
            if (!Players.ContainsKey(player))
                return false;
            int killerOrg = 0;
            if (killer.IsLogged() && Players.ContainsKey(killer))
            {
                ...
                SendKillStatAllPlayers(AddKill(killer.GetCharacter().UUID, killer.Name, Players[killer]));
            }
        ...
        internal override void PlayerDisconnected(Player player)
        {
            if (Players.ContainsKey(player))
                Players.Remove(player);
        }
        ...
        private void EnterToZone(Player player)
        {
        ...
        private void ExitFromZone(Player player)
        {
            if (player.IsInVehicle)
                VehicleManager.WarpPlayerOutOfVehicle(player);
            player.Dimension = 0;
        ...
        private void LoadGameData(Player player)
        {
            ...
            player.TriggerEvent("familyMp:showWarKey", true, WarManager._warPointPositions.GetValueOrDefault(Location)?.Position, WarManager._warPointPositions.GetValueOrDefault(Location)?.Range, Players.ContainsKey(player), (StartDate.AddSeconds(_timeLength) - DateTime.Now).TotalSeconds);
            player.TriggerCefEvent("war/setTittle", $"{(int)_orgType}");
        }

        private void UnloadGameData(Player player)
        {
            player.TriggerEvent("familyMp:showWarKey", false, null, null, false, 0);
        }
    }
}
